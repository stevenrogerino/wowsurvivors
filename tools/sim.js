...
(async () => {
  ...
  const builds = buildTable(roster.weapons, roster.unions);
  ...
  for (const build of builds) {
    const dummy = await page.evaluate(
      ([b, s, n]) => window.__sim(b, 'dummy', s, n), [build, [], DUMMY_SECONDS]);
    const gaunt = await page.evaluate(
      ([b, s, n]) => window.__sim(b, 'gauntlet', s, n), [build, script, GAUNTLET_SECONDS]);
    ...
    const hard = (build.group === 'six' || build.group === 'union')
      ? await page.evaluate(([b, s, n]) => window.__sim(b, 'siege', s, n),
        [build, siege, SIEGE_SECONDS]) : null;
    results.push({

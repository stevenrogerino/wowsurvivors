    ...
    for (let i = 0; i < orbits.count; i++) {
      ...
      for (let n = 0; n < o.count; n++) {
        ...
        if (o.art === 'axe' || o.art === 'sword') {
          /* A first attempt built the head from two curves bulging away
             from a straight back - which is a circle with a flat side, and
             at speed it read as a coin. An axe head is a WEDGE: a concave
             notch at the eye where the haft passes through, straight
             diagonals flaring out to sharp toe and heel corners, and only
             the bit itself - between those corners - genuinely convex. */
          const head = () => {
            ctx.beginPath();
            ctx.moveTo(S * 0.22, -S * 0.28);
            ctx.quadraticCurveTo(S * 0.05, 0, S * 0.22, S * 0.28);
            ctx.lineTo(S * 0.62, S * 0.52);
            ctx.quadraticCurveTo(S * 1.18, S * 0.28, S * 1.24, 0);
            ctx.quadraticCurveTo(S * 1.18, -S * 0.28, S * 0.62, -S * 0.52);
            ctx.closePath();
          };
          const haft = () => {
            ctx.beginPath();
          ...
          ctx.quadraticCurveTo(S * 1.06, -S * 0.24, S * 1.10, 0);
          ctx.quadraticCurveTo(S * 1.06, S * 0.24, S * 0.68, S * 0.46);
          ctx.stroke();
          ctx.restore();
        } else {
          ctx.beginPath();

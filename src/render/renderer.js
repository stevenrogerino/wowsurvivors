    ...
    for (let i = 0; i < orbits.count; i++) {
      ...
      for (let n = 0; n < o.count; n++) {
        ...
        if (o.art === 'axe' || o.art === 'sword') {
          /* A third pass. Two rounds of a wide, roughly AS-TALL-AS-LONG
             wedge both still read as a blob at 20-32px, faceted edge or
             not - and the reason isn't the edge, it's the aspect ratio.
             The dagger this sits beside is a knife shape for exactly one
             reason worth stealing: it is about three times longer than it
             is wide, so its silhouette can only ever be read as elongated,
             never as round. This head is built the same way - a hatchet
             profile, narrow at the eye and reaching well out from the haft
             before it ever gets as wide as it's going to get - rather than
             a flared crescent that is nearly as tall as it is long and so
             looks circular from any distance an anti-aliased 20px shape is
             ever seen at. */
          const head = () => {
            ctx.beginPath();
          ...
          ctx.strokeStyle = 'rgba(8,10,16,.7)';
          ctx.lineWidth = WS.max(1, S * 0.08);
          ctx.beginPath();

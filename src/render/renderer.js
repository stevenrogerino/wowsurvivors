    ...
    for (let i = 0; i < orbits.count; i++) {
      ...
      for (let n = 0; n < o.count; n++) {
        ...
        if (o.art === 'axe' || o.art === 'sword') {
          /* A fifth pass, going back to the two-part structure (a blocky
             poll plus a single wide-flaring blade) rather than the plain
             wedge: closer to an actual battleaxe head, and the earlier
             version of this idea read as a boat only because the poll and
             the blade were two disconnected shapes with a gap between them
             (mast-and-sail) and the blade's outer edge was one smooth
             curve (a hull). Merging them into one continuous outline and
             cutting the curve into straight facets removes both cues
             without giving up the two-part silhouette - a visible panel
             line still separates the block from the blade, so the "two
             parts" read survives, it just isn't drawn as two shapes. */
          const head = () => {
            ctx.beginPath();
            ctx.moveTo(S * 0.10, -S * 0.30);
            ctx.lineTo(S * 0.46, -S * 0.30);
            ctx.lineTo(S * 0.68, -S * 0.12);
            ctx.lineTo(S * 1.20, S * 0.04);
            ctx.lineTo(S * 1.05, S * 0.36);
            ctx.lineTo(S * 0.60, S * 0.56);
          ...
          // the panel line between the poll block and the blade
          ctx.beginPath();
          ctx.moveTo(S * 0.68, -S * 0.12);
          ctx.lineTo(S * 0.28, S * 0.36);
          ctx.stroke();
          // the bevel along the blade's own outer edge
          ctx.beginPath();
          ctx.moveTo(S * 1.20, S * 0.04);
          ctx.lineTo(S * 1.05, S * 0.36);
          ctx.lineTo(S * 0.60, S * 0.56);
          ctx.stroke();
          ctx.restore();

    ...
    for (let i = 0; i < orbits.count; i++) {
      ...
      for (let n = 0; n < o.count; n++) {
        ...
        if (o.art === 'axe' || o.art === 'sword') {
          /* A second pass on this shape: the wedge above was correct in
             principle - concave eye, straight diagonals, sharp corners -
             but the corners it flared to sat so close to the cutting edge's
             own curve that at the size this actually draws at (20-32px,
             not the sketch it was checked at) the toe and heel folded back
             into the bit and the whole head anti-aliased into a round white
             nub on a stick. A wider flare, taken further back down the
             haft before it turns, keeps those corners a real distance from
             the curve between them, so the silhouette survives getting
             small the same way the chest emblem's spiral had to learn to. */
          const head = () => {
            ctx.beginPath();
            ctx.moveTo(S * 0.20, -S * 0.18);
            ctx.quadraticCurveTo(S * 0.04, 0, S * 0.20, S * 0.18);
            ctx.lineTo(S * 0.40, S * 0.24);
            ctx.quadraticCurveTo(S * 0.62, S * 0.80, S * 1.06, S * 0.62);
            ctx.quadraticCurveTo(S * 0.92, S * 0.20, S * 1.00, 0);
            ctx.quadraticCurveTo(S * 0.92, -S * 0.20, S * 1.06, -S * 0.62);
            ctx.quadraticCurveTo(S * 0.62, -S * 0.80, S * 0.40, -S * 0.24);
            ctx.closePath();
          };
          const haft = () => {
            ctx.beginPath();
          ...
          ctx.quadraticCurveTo(S * 0.94, -S * 0.66, S * 0.98, 0);
          ctx.quadraticCurveTo(S * 0.94, S * 0.66, S * 0.50, S * 0.60);
          ctx.stroke();
          ctx.restore();
        } else {
          ctx.beginPath();

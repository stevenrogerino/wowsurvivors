    ...
    for (let i = 0; i < orbits.count; i++) {
      ...
      for (let n = 0; n < o.count; n++) {
        ...
        if (o.art === 'axe' || o.art === 'sword') {
          /* A fourth pass. A symmetric elongated wedge (the previous round)
             fixed the round-blob problem but still read as generic, not as
             an axe specifically - and every attempt to widen it back out
             for a more axe-like silhouette immediately collapsed back into
             a blob, a leaf, or (with a flat top and a curved bottom both
             trying to read at once) a little sailboat. What actually reads
             as an axe head at this size is a single-bit profile: a
             straight, near-flat SPINE on one edge, and a distinctly wider,
             faceted cutting edge on the other - built from straight
             segments throughout, because every curve tried here softened
             into round at 20-32px the same way the mustache's curl did at
             gameplay size. Checked un-rotated and large before it was ever
             checked small, specifically to catch that collapse early
             rather than after porting it in. */
          const head = () => {
            ctx.beginPath();
            ctx.moveTo(S * 0.15, -S * 0.10);
            ctx.lineTo(S * 1.25, -S * 0.14);
            ctx.lineTo(S * 1.50, S * 0.02);
            ctx.lineTo(S * 1.30, S * 0.35);
            ctx.lineTo(S * 0.95, S * 0.55);
            ctx.lineTo(S * 0.55, S * 0.42);
            ctx.lineTo(S * 0.20, S * 0.15);
            ctx.closePath();
          };
          const haft = () => {
            ctx.beginPath();

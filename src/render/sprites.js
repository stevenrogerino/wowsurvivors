  ...
  const CREATURES = {
    ...
    bearform(g, s, p) {
      /* THE BEAR, as a survivor wears it: down on all fours and FAT - a
         ...
         low and forward with the mouth open. (It stood upright once, to
         read apart from the horde that walks on four; a tester wanted it on
         all fours and heavy, and it is still told from anything coming at
      ...
      const paw = (x, y, pal, w) => {
        shaded(g, X(x), Y(y), X(w), X(w * 0.5), pal);
        for (let k = 0; k < 4; k++) {
          const tx = x - w * 0.78 + k * w * 0.5, ty = y + w * 0.2;
          shaded(g, X(tx), Y(ty), X(w * 0.26), X(w * 0.22), pal);
          // the claw: a crescent hooked forward (left) and down to the ground
          g.fillStyle = '#efe6d2'; g.strokeStyle = '#4a4032'; g.lineWidth = u * 0.35;
          g.beginPath();
          g.moveTo(X(tx - w * 0.14), Y(ty + w * 0.08));
          g.quadraticCurveTo(X(tx - w * 0.5), Y(ty + w * 0.18), X(tx - w * 0.62), Y(ty + w * 0.52));
          g.quadraticCurveTo(X(tx - w * 0.3), Y(ty + w * 0.3), X(tx + w * 0.1), Y(ty + w * 0.2));
      ...
      g.save(); body(); g.clip();
      pelt(g, p, X(58), X(56), X(32), X(26), 0.05, 9, 0.35, 0.5);
      ...
      g.beginPath(); g.moveTo(X(72), Y(50)); g.quadraticCurveTo(X(80), Y(60), X(74), Y(72)); g.stroke();
      g.strokeStyle = p.hi; g.globalAlpha = 0.35; g.lineWidth = u;
      g.beginPath(); g.moveTo(X(34), Y(40)); g.quadraticCurveTo(X(50), Y(28), X(70), Y(35)); g.stroke();
      // the Old Shapes' claw marks, glowing in the fur of the flank
      g.globalAlpha = 1;
      g.save();
      g.shadowColor = '#9cf06a'; g.shadowBlur = X(3);
      g.strokeStyle = 'rgba(170,250,120,.9)'; g.lineWidth = u * 1.3;
      for (let k = 0; k < 3; k++) {
        g.beginPath(); g.moveTo(X(58 + k * 4), Y(42 + k * 0.4)); g.quadraticCurveTo(X(61 + k * 4), Y(50), X(59 + k * 4), Y(58)); g.stroke();
      }
      g.restore();
      g.restore();
      ...
      for (const [x, y] of [[18, 36], [30, 34.4]]) {
        shaded(g, X(x), Y(y), X(4.6), X(4.2), p);
        g.fillStyle = 'rgba(60,30,20,.6)';
        g.beginPath(); g.ellipse(X(x), Y(y + 0.7), X(2.3), X(2.1), 0, 0, WS.TAU); g.fill();
      }
      // the totem necklace at the throat
      g.strokeStyle = '#3a2a1a'; g.lineWidth = u * 0.8;
      g.beginPath(); g.moveTo(X(26), Y(58)); g.quadraticCurveTo(X(34), Y(66), X(40), Y(56)); g.stroke();
      for (let k = 0; k < 5; k++) {
        const t = k / 4, x = 27 + t * 12, y = 59 + Math.sin(t * WS.PI) * 5 - t * 2;
        if (k % 2) shaded(g, X(x), Y(y + 0.6), X(1.2), X(1.2), { hi: '#9ae0c0', mid: '#4aa080', lo: '#2a6048', dark: '#1a3a2a', line: '#10241a', glow: '#fff' });
        else poly(g, P([[x - 0.9, y], [x + 0.9, y], [x + 0.2, y + 3.2]]), '#f2ecdc', '#6a6254', u * 0.35);
      }
      poly(g, P([[31.4, 63], [34.6, 63], [35, 67.4], [33, 69.2], [31, 67.4]]), '#a8784a', '#3a2412', u * 0.5);
      // the muzzle, the jaw dropped open
      mass(g, P([[18, 44], [7, 46], [3.6, 50.6], [7, 54], [18, 54]]), pale);
      poly(g, P([[5.6, 53.4], [17.4, 54], [16.4, 60.4], [8.6, 60.4], [5, 57]]), '#3a1614', '#140606', u * 0.6);
      poly(g, P([[8.4, 57.6], [15, 57.4], [14.2, 59.6], [9.2, 59.8]]), '#d86a6e', '#6a2020', u * 0.4);
      for (const [tx, dn] of [[6.8, 1], [9.4, 1], [12, 1], [14.6, 1], [8.2, -1], [13.2, -1]]) {
        const ty = dn > 0 ? 53.8 : 60.2;
        poly(g, P([[tx - 0.9, ty], [tx + 0.9, ty], [tx, ty + dn * 2]]), '#fffaf0', '#8a847a', u * 0.3);
      }
      mass(g, P([[5, 57.4], [17, 59], [16.4, 62.6], [8, 63], [4.6, 60.4]]), p);
      g.fillStyle = '#140c0a';
      g.beginPath(); g.ellipse(X(4.6), Y(47.6), X(2.4), X(1.8), -0.2, 0, WS.TAU); g.fill();

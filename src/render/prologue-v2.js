...
(function (WS) {
   ...
   *  drops the frame the sun comes up. Two runs each, now - the bare rig the
   *  roster stands in, and the fully-earned one it flashes into just before
  ...
  function warmCast() {
    ...
    for (const c of cast) {
      ...
      WS.Sprites.hero(ch.art || c.id, ch.color, size, false, undefined, null, WS.Hero.maxRank);
    }
  }
  ...
  function roster(ctx, t, p) {
    ...
    for (const c of order) {
      ...
      if (known) {
        ...
        /* A FLASH into what they become, right as the light is already taking
           them. The crest only ever showed the bare rank-0 rig - the sprite it
           was baked with never carried a rank - so ten survivors the player has
           actually built out arrived looking exactly as new as the ones they
           have not unlocked yet. One beat of the fully-earned kit here says
           what is waiting for them in the game itself.
           A sine over the same window `out` fades on: zero at the window's own
           start and end, so the flash never outlives the figure it belongs to,

...
(function (WS) {
  ...
  function buildScenery() {
     ...
     *
     * Two things fix that and they are both about air. Real ranges are cut
     * into peaks and valleys with sky between them, so these alternate peak,
     ...
     * of it - so each range is filled with a gradient that is palest at its
     * base, where the haze pools, and the far range is paler than the near
     * one. Value going the wrong way is what makes a hill look like a wall. */
    ranges = [
  ...
  const BEATS = {
    night(ctx, t, k, env) {
      ...
      sky(ctx, t, l); ridgeline(ctx, l);
      wood(ctx, woodFar, WOOD_FAR);
      ground(ctx, l);
    ...
    dawn(ctx, t, k, env) {
      ...
      sunrise(ctx, l);
      ridgeline(ctx, l);
      wood(ctx, woodFar, mix(WOOD_FAR, '#3a2432', l));
      ground(ctx, l);
      ...
      wood(ctx, woodNear, mix('#020407', '#150c14', l));
      roster(ctx, t, crest);
   ...
   *
   * It used to take the scene index and the scene's own k, which meant it
   * pushed in across a scene and then SNAPPED back to its start at the cut -
   * nine hard camera resets in fifty-two seconds. Measured by stepping the
   * piece at 15fps and diffing consecutive frames, the night-to-night
   * boundary - where nothing else in the picture changes at all - moved the
   * frame four times as much as a typical step. That was the camera and
   * nothing else.
   *
   * It is a function of absolute time now: one continuous push-in for the
   * whole piece with a slow lateral wander over it, on periods long enough
   * that no cycle ever repeats inside the running time. Because both the
   * outgoing and incoming beats of a dissolve are drawn at the same t, they
   * now share the camera exactly, so a dissolve is purely a change of
   * content. */
  ...
  P.render = function (ctx, time) {
    ...
    /* Softer than it was. A heavy vignette plus a dark unbroken mass along the
       horizon is how a landscape turns into a cave mouth; the ranges above
       carry the haze now, and this only has to close the frame. */
    const v = ctx.createRadialGradient(W / 2, H * 0.44, H * 0.34, W / 2, H * 0.44, H * 0.94);
    v.addColorStop(0, 'rgba(0,0,0,0)');
    v.addColorStop(0.62, 'rgba(0,0,0,.16)');

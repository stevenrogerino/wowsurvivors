...
(function (WS) {
  ...
  function drawHead(g, cfg, C) {
    ...
    if (cfg.horns) {
      ...
      for (const dir of [-1, 1]) {
        ...
        g.beginPath();
        g.moveTo(bx, by);
        g.bezierCurveTo(cx + dir * 13, by - 1, cx + dir * 19, by - 5, cx + dir * 20, CEIL + 1);
        g.bezierCurveTo(cx + dir * 15, by - 8, cx + dir * 12, by - 4, cx + dir * 9.5, by - 4.5);
        g.closePath(); g.fill();
      }
    }
    if (cfg.halo) {
  ...
  const WEAPONS = {
    ...
    totem(g, C, b) {
      const gx = 50 + b.sh + 4;
      limb(g, gx, FOOT_Y - 3, gx + 2, CEIL + 12, 5.4, WOOD);
      for (let i = 0; i < 3; i++) {
        g.fillStyle = i === 1 ? C.accent : GOLD.core;
        g.fillRect(gx - 2.6 + i * 0.3, CEIL + 16 + i * 11, 5.6, 3.4);
      }
      glow(g, gx + 1, CEIL + 14, 8, C.accent, 0.7);
    },
    shield(g, C, b) {
      const sx = 50 - b.sh - 4;
    ...
    glaives(g, C, b) {
      for (const dir of [-1, 1]) {
        g.save();
        g.translate(50 + dir * (b.sh + 7), WAIST_Y - 8);
        g.rotate(dir * 0.4);
        g.strokeStyle = C.accentLight; g.lineWidth = 2.2; g.lineCap = 'round';
        g.beginPath(); g.arc(0, 0, 9.5, -1.0, 1.5); g.stroke();
        g.restore();
        glow(g, 50 + dir * (b.sh + 7), WAIST_Y - 8, 13, C.accent, 0.5);
      }
    },
  };
  ...
  const CAST = {
    ...
    shaman: { build: 'normal', head: 'bare', cloak: 'tattered', weapon: 'totem', tabard: true, hair: ramp([0.22, 0.24, 0.30]) },
    ...
    ruinseeker: { build: 'normal', head: 'bare', horns: true, cloak: 'short', weapon: 'glaives', hair: DARKCLOTH },
  };
  ...
  function colours(tint, demon) {
    ...
    return {
      ...
      hornRamp: demon ? ramp([0.42, 0.62, 0.28]) : ramp([0.33, 0.30, 0.34]),

...
(function (WS) {
  ...
  /* WHO IS DRAWN. Set once per head by drawHead from the kit, and read by
   * the skull, the eyes and the mouth, which all need it and none of which
   * is handed the kit. The women of the Watch get a narrower chin, lashes,
   * a finer arched brow and a mouth with colour in it; the men a squarer
   * jaw, a heavier level brow and a shadow of stubble where there is no
   * beard. Small marks, all inside the outline, and together they are the
   * difference between twelve people and twelve of the same doll. */
  let FEM = false;
  let JAW = 0.42, JAW_Y = 0.66;
  ...
  function skull(g, cx) {
    ...
    g.bezierCurveTo(cx + HEAD_RX, HEAD_CY + HEAD_RY * JAW_Y, cx + HEAD_RX * JAW, HEAD_CY + HEAD_RY,
      cx, HEAD_CY + HEAD_RY);
    g.bezierCurveTo(cx - HEAD_RX * JAW, HEAD_CY + HEAD_RY, cx - HEAD_RX, HEAD_CY + HEAD_RY * JAW_Y,
      cx - HEAD_RX, HEAD_CY - 2);
    g.closePath();
  }
  ...
  function eyes(g, cx, y, C, tight, lit2) {
    ...
    for (const dir of [-1, 1]) {
      ...
      if (tight) {
        ...
        g.fillStyle = 'rgba(44,30,22,.3)';
        g.beginPath(); g.ellipse(0, -0.2, 3.0, 2.1, 0, 0, WS.TAU); g.fill();
        /* AN EYE, not a hole. The dark almond carried the look at 34px and
           still does - it is the base everything below sits in - but at
           portrait size it was a blank: no lid, no iris, the whole cast
           staring out of the same two dolls' eyes. So inside it, small
           enough that the mark stays dark at play size: a sliver of white
           at the corners, an iris in the survivor's own colour, a pupil, a
           catchlight, and a heavy upper lid that gives the eye its lean. */
        g.fillStyle = '#20161a';
        g.beginPath(); g.ellipse(0, 0, 2.15, 1.6, 0, 0, WS.TAU); g.fill();
        g.save();
        g.beginPath(); g.ellipse(0, 0.1, 2.0, 1.42, 0, 0, WS.TAU); g.clip();
        g.fillStyle = 'rgba(236,226,214,.55)';
        g.fillRect(-2.2, -1.6, 4.4, 3.2);
        g.fillStyle = C.irisRing || '#2a1c14';
        g.beginPath(); g.arc(dir * -0.15, 0.15, 1.3, 0, WS.TAU); g.fill();
        g.fillStyle = C.iris || C.eye;
        g.beginPath(); g.arc(dir * -0.15, 0.15, 1.02, 0, WS.TAU); g.fill();
        g.fillStyle = '#0e0a0c';
        g.beginPath(); g.arc(dir * -0.15, 0.2, 0.52, 0, WS.TAU); g.fill();
        // the lid's shadow across the top of the eyeball
        const lid = g.createLinearGradient(0, -1.6, 0, 0.2);
        lid.addColorStop(0, 'rgba(30,18,14,.75)'); lid.addColorStop(1, 'rgba(30,18,14,0)');
        g.fillStyle = lid; g.fillRect(-2.2, -1.6, 4.4, 1.8);
        g.restore();
        g.fillStyle = 'rgba(255,255,255,.92)';
        g.beginPath(); g.arc(dir * -0.15 - 0.45, -0.3, 0.36, 0, WS.TAU); g.fill();
        // the upper lid: a heavy line over the top, flicked at the outer end
        g.strokeStyle = '#1a100c'; g.lineWidth = 0.85; g.lineCap = 'round';
        g.beginPath();
        g.moveTo(-dir * 2.1, 0.2);
        g.quadraticCurveTo(0, -2.1, dir * 2.3, -0.3);
        g.lineTo(dir * 2.9, -0.7);
        g.stroke();
        if (FEM) {
          // lashes: three flicks off the outer half of the lid, the longest last
          g.lineWidth = 0.55;
          for (const [lx, ly, ex, ey] of [[0.7, -1.35, 1.1, -2.5], [1.6, -1.0, 2.4, -2.1], [2.4, -0.5, 3.6, -1.3]]) {
            g.beginPath(); g.moveTo(dir * lx, ly); g.lineTo(dir * ex, ey); g.stroke();
          }
        }
      }
      g.restore();
    ...
    /* Set low at the inner end and arched out, which is resolve; the other
       way up - inner end high - read, once the eyes had lids, as worry. */
    g.strokeStyle = tight ? 'rgba(26,18,12,.45)' : 'rgba(34,22,14,.7)';
    g.lineWidth = tight ? 1.1 : FEM ? 0.8 : 1.7; g.lineCap = 'round';
    for (const dir of [-1, 1]) {
      g.beginPath();
      if (!tight && FEM) {
        // finer, and arched higher: the whole of a softer look is in this
        g.moveTo(cx + dir * 2.0, y - 3.4);
        g.quadraticCurveTo(cx + dir * 4.4, y - 5.5, cx + dir * 6.6, y - 3.9);
      } else if (!tight) {
        // heavy, low and nearly level: a brow that has frowned for years
        g.moveTo(cx + dir * 1.5, y - 2.6);
        g.quadraticCurveTo(cx + dir * 4.2, y - 3.9, cx + dir * 6.7, y - 3.4);
      } else {
        g.moveTo(cx + dir * 1.8, y - 2.9);
        g.quadraticCurveTo(cx + dir * 4.2, y - 4.4, cx + dir * 6.4, y - 3.6);
      }
      g.stroke();
    }
    if (!tight) {
  ...
  /** A mouth, and the warmth in the cheeks. Only on the open faces - a
   *  beard or a moustache already is the lower half of the face.
   *
   *  Every clean-shaven survivor was eyes and a nose over an unbroken
   *  field of skin down to the chin, which is the face of a doll. Two
   *  marks change that: a mouth set a little firm (they have been awake
   *  all night) with the lower lip catching the light, and a blush on each
   *  cheek. Neither changes a pixel of the outline. */
  function face(g, cx, y) {
    const my = y + 7.4;
    g.save();
    for (const dir of [-1, 1]) {
      const bl = g.createRadialGradient(cx + dir * 6.2, y + 4.4, 0, cx + dir * 6.2, y + 4.4, 3.4);
      const a = FEM ? 0.36 : 0.14;
      bl.addColorStop(0, `rgba(214,110,96,${a})`); bl.addColorStop(1, 'rgba(214,110,96,0)');
      g.fillStyle = bl;
      g.beginPath(); g.arc(cx + dir * 6.2, y + 4.4, 3.4, 0, WS.TAU); g.fill();
    }
    g.lineCap = 'round';
    if (FEM) {
      /* A mouth with colour in it: a bowed upper lip and a fuller lower one,
         a line between them, and one catch of light on the lower lip. */
      g.fillStyle = 'rgba(186,70,84,.9)';
      g.beginPath();
      g.moveTo(cx - 2.5, my);
      g.quadraticCurveTo(cx - 1.2, my - 1.3, cx, my - 0.6);
      g.quadraticCurveTo(cx + 1.2, my - 1.3, cx + 2.5, my);
      g.quadraticCurveTo(cx, my + 2.4, cx - 2.5, my);
      g.closePath(); g.fill();
      g.strokeStyle = 'rgba(96,26,34,.8)'; g.lineWidth = 0.5;
      g.beginPath(); g.moveTo(cx - 2.4, my + 0.05); g.quadraticCurveTo(cx, my + 0.7, cx + 2.4, my + 0.05); g.stroke();
      g.fillStyle = 'rgba(255,214,214,.55)';
      g.beginPath(); g.ellipse(cx - 0.3, my + 1.1, 0.9, 0.35, 0, 0, WS.TAU); g.fill();
      g.restore();
      return;
    }
    /* Stubble: a shadow along the jaw and round the mouth, clipped to the
       face, with a little grain in it so it reads as hair and not dirt. */
    g.save();
    skull(g, cx); g.clip();
    const st = g.createLinearGradient(0, y + 3.5, 0, HEAD_CY + HEAD_RY);
    st.addColorStop(0, 'rgba(46,36,32,0)'); st.addColorStop(0.45, 'rgba(46,36,32,.22)');
    st.addColorStop(1, 'rgba(46,36,32,.34)');
    g.fillStyle = st;
    g.fillRect(cx - HEAD_RX, y + 3.5, HEAD_RX * 2, HEAD_RY);
    g.fillStyle = 'rgba(40,30,26,.35)';
    for (let i = 0; i < 26; i++) {
      const a = (i * 2.399) % WS.TAU, r = 3.5 + (i * 7 % 11) * 0.5;
      const px = cx + WS.cos(a) * r * 0.95, py = my + 1.2 + WS.abs(WS.sin(a)) * r * 0.42;
      g.fillRect(px, py, 0.35, 0.35);
    }
    g.restore();
    g.strokeStyle = 'rgba(70,32,26,.8)'; g.lineWidth = 0.8;
    // a half smile, the right corner up: they have made it through worse
    g.beginPath();
    g.moveTo(cx - 2.4, my - 0.3);
    g.quadraticCurveTo(cx - 0.2, my + 0.7, cx + 2.7, my - 0.8);
    g.stroke();
    // the lower lip: a little light under the line, and its shadow beneath
    g.strokeStyle = 'rgba(236,170,150,.5)'; g.lineWidth = 0.7;
    g.beginPath(); g.moveTo(cx - 1.2, my + 1.0); g.quadraticCurveTo(cx + 0.2, my + 1.5, cx + 1.4, my + 1.0); g.stroke();
    g.strokeStyle = 'rgba(40,22,16,.25)'; g.lineWidth = 0.6;
    g.beginPath(); g.moveTo(cx - 1, my + 2.2); g.quadraticCurveTo(cx + 0.2, my + 2.6, cx + 1.2, my + 2.2); g.stroke();
    g.restore();
  }
  ...
  function drawHead(g, cfg, C) {
    ...
    if (cfg.undead) {
      ...
      if (cfg.hoodFace) {
        /* Rav's capelet: cut ragged at the bottom and lit along the top, so
           a dark cowl over dark cloth is still two things. A clean trapezoid
           in a hood this dark was a black slab across the chest. */
        panel(g, [[cx - 17, mantle - 3], [cx + 15, mantle - 3], [cx + 12.5, mantle + 6.5],
          [cx + 8, mantle + 3.6], [cx + 4, mantle + 8], [cx - 0.5, mantle + 4.2],
          [cx - 5, mantle + 8.6], [cx - 9.5, mantle + 4.6], [cx - 14, mantle + 7]], hr);
        g.save();
        g.strokeStyle = 'rgba(0,0,0,.35)'; g.lineWidth = 0.8;
        for (const x of [cx + 8, cx - 0.5, cx - 9.5]) {
          g.beginPath(); g.moveTo(x, mantle - 2); g.lineTo(x + 0.4, mantle + 3.6); g.stroke();
        }
        g.restore();
      } else {
      ...
      g.clip();
      if (cfg.hoodFace) {
        /* Rav: the dark back in the hood, and the gang's red over the face.
           For a while he showed a plain face above the kerchief, which told
           the rogue from the warlock but lost the menace; the warlock's void
           is a round glow, his is two narrow slits cut low and angled in, and
           the bandana tied over the lower half says outlaw before anything
           else about him does. */
        const void_ = g.createRadialGradient(cx, HEAD_CY + 3, 1, cx, HEAD_CY + 1.5, 10);
        void_.addColorStop(0, 'rgba(5,6,10,1)');
        void_.addColorStop(0.72, 'rgba(5,6,10,.98)');
        void_.addColorStop(1, 'rgba(5,6,10,.6)');
        g.fillStyle = void_;
        g.fillRect(cx - 10, HEAD_CY - 10, 20, 22);
        g.restore();
        for (const dir of [-1, 1]) {
          g.save();
          g.translate(cx + dir * 3.7, eyeY - 0.6);
          g.scale(dir, 1);
          // a slit whose top edge drops toward the nose: a glare, not a glow
          const slit = () => {
            g.beginPath();
            g.moveTo(-2.1, 0.5);
            g.lineTo(2.6, -1.5);
            g.quadraticCurveTo(1.4, 1.3, -2.1, 0.5);
            g.closePath();
          };
          g.fillStyle = C.eye;
          slit(); g.fill();
          g.globalAlpha = 0.8; g.fillStyle = '#fffbe8';
          g.beginPath(); g.ellipse(0.4, -0.2, 0.9, 0.35, -0.35, 0, WS.TAU); g.fill();
          g.restore();
          glow(g, cx + dir * 3.9, eyeY - 0.8, 5.5, C.eye, 0.55);
        }
        const red = ramp([0.62, 0.11, 0.11]);
        // the tails of the knot, flicking out from behind the far temple
        panel(g, [[cx - 8.5, eyeY + 2.6], [cx - 15.5, eyeY + 4.2], [cx - 17.2, eyeY + 8.8],
          [cx - 13, eyeY + 6.6], [cx - 9.5, eyeY + 5.4]], ramp([0.5, 0.09, 0.09]));
        panel(g, [[cx - 9, eyeY + 3.6], [cx - 13.6, eyeY + 9.4], [cx - 12.2, eyeY + 12.6],
          [cx - 10.6, eyeY + 8.6]], ramp([0.44, 0.08, 0.08]));
        // the mask: up over the bridge of the nose, down to a point on the chest
        const mask = [[cx - 9.6, eyeY + 2.9], [cx - 3, eyeY + 1.9], [cx, eyeY + 1.2], [cx + 3, eyeY + 1.9],
          [cx + 9.4, eyeY + 2.6], [cx + 8.6, HEAD_CY + 8.4], [cx + 1, NECK_Y + 6.5],
          [cx - 8.8, HEAD_CY + 8.8]];
        panel(g, mask, red);
        g.save();
        g.beginPath(); g.moveTo(mask[0][0], mask[0][1]);
        for (let i = 1; i < mask.length; i++) g.lineTo(mask[i][0], mask[i][1]);
        g.closePath(); g.clip();
        // folds converging on the point, and the ridge of the nose under it
        g.strokeStyle = red.line; g.globalAlpha = 0.6; g.lineWidth = 0.7;
        g.beginPath();
        g.moveTo(cx - 7.5, eyeY + 4.4); g.quadraticCurveTo(cx - 3, eyeY + 8.6, cx + 0.6, NECK_Y + 5.6);
        g.moveTo(cx + 7.2, eyeY + 4.2); g.quadraticCurveTo(cx + 3.2, eyeY + 8.2, cx + 1.2, NECK_Y + 5.4);
        g.moveTo(cx - 3.6, HEAD_CY + 7.6); g.quadraticCurveTo(cx, HEAD_CY + 9.4, cx + 3.6, HEAD_CY + 7.4);
        g.stroke();
        g.strokeStyle = red.key; g.globalAlpha = 0.45; g.lineWidth = 0.8;
        g.beginPath(); g.moveTo(cx - 8.4, eyeY + 3.3); g.lineTo(cx, eyeY + 1.8); g.lineTo(cx + 8.2, eyeY + 3.1); g.stroke();
        // the pattern: white dots and a paisley tear or two
        g.globalAlpha = 0.72; g.fillStyle = '#f2e6d8';
        for (const [dx, dy] of [[-5.6, 4.6], [-1.8, 5.2], [2.2, 4.8], [5.8, 4.6], [-3.6, 8.2], [0.4, 8.8],
          [4.2, 8], [-1.6, 11.8], [2.2, 12.2], [0.6, 15.2]]) {
          g.beginPath(); g.arc(cx + dx, eyeY + dy, 0.5, 0, WS.TAU); g.fill();
        }
        g.globalAlpha = 0.55; g.strokeStyle = '#f2e6d8'; g.lineWidth = 0.45;
        for (const [dx, dy, rot] of [[-6, 8.4, 0.6], [6.2, 8, -0.6]]) {
          g.save(); g.translate(cx + dx, eyeY + dy); g.rotate(rot);
          g.beginPath(); g.arc(0, 0, 1.1, 0.3, WS.TAU - 0.9); g.lineTo(1.9, 1.2); g.stroke();
          g.restore();
        }
        g.restore();
        // the knot itself, at the far temple, over the hood's edge
        blob(g, cx - 9.2, eyeY + 3.6, 1.9, 1.5, red);
      } else {
        const void_ = g.createRadialGradient(cx, HEAD_CY + 3, 1, cx, HEAD_CY + 1.5, 10);
      ...
      if (helmed) {
        ...
        eyes(g, cx, eyeY, C, false, false);
        if (!cfg.beard && !cfg.mustache) face(g, cx, eyeY);
        if (cfg.goatee) {
          /* A sage's beard: a single long point off the chin, grey going to
             white, the one thing on his head he has let grow. */
          const gr = cfg.goatee, top = eyeY + 9.2, tip = NECK_Y + 9;
          lit(g, gr, cx - 3.4, top, cx + 3.4, tip);
          const goat = () => {
            g.beginPath();
            g.moveTo(cx - 3.4, top);
            g.quadraticCurveTo(cx - 3.8, top + 5.5, cx + 0.8, tip);
            g.quadraticCurveTo(cx + 3.6, top + 5.5, cx + 3.2, top);
            g.quadraticCurveTo(cx, top + 1.4, cx - 3.4, top);
            g.closePath();
          };
          goat(); g.fill();
          g.save(); goat(); g.clip();
          g.strokeStyle = gr.line; g.globalAlpha = 0.5; g.lineWidth = 0.45;
          for (let i = -2; i <= 2; i++) {
            g.beginPath(); g.moveTo(cx + i * 1.2, top + 0.6); g.quadraticCurveTo(cx + i * 1.1, top + 5, cx + 0.8 + i * 0.3, tip - 0.5); g.stroke();
          }
          g.restore();
          g.strokeStyle = gr.line; g.lineWidth = 0.6; g.globalAlpha = 0.7;
          goat(); g.stroke();
          g.globalAlpha = 1;
        }
        if (cfg.beard) {
        ...
        if (cfg.mustache) {
          ...
          for (const dir of [-1, 1]) {
            ...
            g.beginPath(); g.arc(lastX, lastY, 1.05, 0, WS.TAU); g.fill();
            g.restore();
          }
  ...
  function bovineHead(g, cfg, C, cx, eyeY) {
    ...
    // eyes: big, gentle, set wide - and on her, lashed
    for (const dir of [-1, 1]) {
      g.fillStyle = 'rgba(14,10,8,.95)';
      g.beginPath(); g.ellipse(cx + dir * 6, eyeY - 1.5, 1.9, 2.2, 0, 0, WS.TAU); g.fill();
      g.fillStyle = 'rgba(92,58,40,.9)';
      g.beginPath(); g.arc(cx + dir * 6, eyeY - 1.2, 1.1, 0, WS.TAU); g.fill();
      g.fillStyle = 'rgba(14,10,8,.95)';
      g.beginPath(); g.arc(cx + dir * 6, eyeY - 1.2, 0.55, 0, WS.TAU); g.fill();
      g.fillStyle = 'rgba(255,255,255,.9)';
      g.beginPath(); g.arc(cx + dir * 6 - 0.6, eyeY - 2.3, 0.6, 0, WS.TAU); g.fill();
      g.strokeStyle = 'rgba(14,10,8,.9)'; g.lineWidth = 0.7; g.lineCap = 'round';
      if (cfg.fem) {
        g.beginPath(); g.ellipse(cx + dir * 6, eyeY - 1.5, 2.0, 2.3, 0, WS.PI * 1.05, WS.PI * 1.95); g.stroke();
        g.lineWidth = 0.6;
        for (const [a, l] of [[-0.3, 1.6], [0.25, 1.9], [0.8, 2.1]]) {
          const ang = -WS.PI / 2 + dir * a;
          const x0 = cx + dir * 6 + WS.cos(ang) * 2.1, y0 = eyeY - 1.5 + WS.sin(ang) * 2.3;
          g.beginPath(); g.moveTo(x0, y0);
          g.lineTo(x0 + WS.cos(ang) * l + dir * 0.5, y0 + WS.sin(ang) * l); g.stroke();
        }
      } else {
        g.globalAlpha = 0.6;
    ...
    /* A bull wears a ring; she wears daisies. A blush either side of the
       muzzle, a smile under it, and a crown of flowers from the pasture
       across the poll between the horns. */
    for (const dir of [-1, 1]) {
      const bl = g.createRadialGradient(cx + dir * 7.8, my - 2.6, 0, cx + dir * 7.8, my - 2.6, 3);
      bl.addColorStop(0, 'rgba(236,120,140,.55)'); bl.addColorStop(1, 'rgba(236,120,140,0)');
      g.fillStyle = bl;
      g.beginPath(); g.arc(cx + dir * 7.8, my - 2.6, 3, 0, WS.TAU); g.fill();
    }
    g.strokeStyle = 'rgba(80,30,34,.75)'; g.lineWidth = 0.6; g.lineCap = 'round';
    g.beginPath(); g.moveTo(cx - 2.6, my + 2.4); g.quadraticCurveTo(cx, my + 3.9, cx + 2.6, my + 2.4); g.stroke();
    const daisy = (x, y, r, hub) => {
      g.fillStyle = '#fbf7ec';
      for (let k = 0; k < 6; k++) {
        const a = k / 6 * WS.TAU;
        g.beginPath(); g.ellipse(x + WS.cos(a) * r * 0.62, y + WS.sin(a) * r * 0.62, r * 0.48, r * 0.26, a, 0, WS.TAU); g.fill();
      }
      g.fillStyle = hub;
      g.beginPath(); g.arc(x, y, r * 0.34, 0, WS.TAU); g.fill();
    };
    const top = hy - ry + 0.6;
    g.strokeStyle = 'rgba(74,120,52,.9)'; g.lineWidth = 0.9;
    g.beginPath(); g.moveTo(cx - 10, top + 2.8); g.quadraticCurveTo(cx, top - 3.2, cx + 10, top + 2.8); g.stroke();
    for (const [dx, dy, r, hub] of [[-9, 2.2, 1.9, '#f2c230'], [-4.6, -0.6, 2.1, '#f2c230'], [0, -1.6, 2.4, '#ef8fb0'],
      [4.6, -0.6, 2.1, '#f2c230'], [9, 2.2, 1.9, '#f2c230']]) {
      daisy(cx + dx, top + dy, r, hub);
    }
  }
  ...
  function drawBody(g, cfg, C, b, w, lift) {
    ...
    if (cfg.robe) {
      ...
      g.restore();
      /* Embroidery above the braid: a running border of diamonds in the
         survivor's own colour, stitched between two threads. A robe is the
         most expensive thing a caster owns and it was plain cloth to the
         floor; a worked border is what says somebody made it for them. */
      g.save();
      g.beginPath();
      ...
      g.globalAlpha = 0.85;
      for (let i = 0; i < 9; i++) {
        const x = x0 + span * (i + 0.5) / 9;
        g.fillStyle = C.accentRamp.core;
        g.beginPath(); g.moveTo(x, ey - 1.6); g.lineTo(x + 1.3, ey); g.lineTo(x, ey + 1.6); g.lineTo(x - 1.3, ey); g.closePath(); g.fill();
        g.fillStyle = C.trimRamp.key;
        g.beginPath(); g.arc(x + span / 18, ey, 0.45, 0, WS.TAU); g.fill();
      }
      g.restore();
      ...
      g.restore();
      if (cfg.apron) {
        /* A milkmaid's apron over the robe: pale linen from the belt to
           just above the hem, a scalloped lace edge, one pocket and a
           daisy stitched on it. Nothing else in the cast wears white below
           the waist, so it is hers at any size. */
        const ax0 = b.waist + 0.5, ax1 = b.hip + 3.5, ay0 = WAIST_Y - 2 - lift, ay1 = FOOT_Y - 9;
        const apr = ramp([0.92, 0.90, 0.85]);
        const d2 = w.swing * 2.5;
        panel(g, [[cx - ax0, ay0], [cx + ax0, ay0], [cx + ax1 + d2, ay1], [cx - ax1 + d2, ay1]], apr);
        g.fillStyle = apr.key;
        for (let k = 0; k < 7; k++) {
          const x = cx - ax1 + d2 + (k + 0.5) * (ax1 * 2 / 7);
          g.beginPath(); g.arc(x, ay1, ax1 / 7, 0, WS.PI); g.fill();
        }
        g.strokeStyle = apr.line; g.globalAlpha = 0.5; g.lineWidth = 0.6;
        for (let k = 0; k < 7; k++) {
          const x = cx - ax1 + d2 + (k + 0.5) * (ax1 * 2 / 7);
          g.beginPath(); g.arc(x, ay1, ax1 / 7, 0, WS.PI); g.stroke();
        }
        // two soft folds
        for (const u of [-0.45, 0.4]) {
          g.beginPath(); g.moveTo(cx + u * ax0, ay0 + 2); g.lineTo(cx + u * ax1 + d2, ay1 - 1); g.stroke();
        }
        g.globalAlpha = 1;
        const px = cx + 3 + d2 * 0.5, py = ay0 + 9;
        panel(g, [[px - 3.4, py], [px + 3.4, py], [px + 3.2, py + 6], [px - 3.2, py + 6]], ramp([0.86, 0.84, 0.79]));
        g.fillStyle = '#f2c230';
        g.beginPath(); g.arc(px, py + 3, 0.7, 0, WS.TAU); g.fill();
        g.fillStyle = '#ef8fb0';
        for (let k = 0; k < 5; k++) {
          const a = k / 5 * WS.TAU;
          g.beginPath(); g.arc(px + WS.cos(a) * 1.3, py + 3 + WS.sin(a) * 1.3, 0.6, 0, WS.TAU); g.fill();
        }
      }
    } else {
      for (const dir of [-1, 1]) {
    ...
    const torso = cfg.fem
      ? [[cx - b.sh, SHOULDER_Y - 2], [cx + b.sh, SHOULDER_Y - 2], [cx + b.sh - 0.4, SHOULDER_Y + 7],
        [cx + b.waist, WAIST_Y], [cx - b.waist, WAIST_Y], [cx - b.sh + 0.4, SHOULDER_Y + 7]]
      : [[cx - b.sh, SHOULDER_Y - 2], [cx + b.sh, SHOULDER_Y - 2],
    ...
    if (!cfg.pauldrons) {
      ...
      g.closePath(); g.clip();
      if (cfg.fem) {
        /* A bust, drawn the way everything else here is: light on the upper
           curve and a shadow under it. No outline - the form is in the
           value change, and it stays inside the silhouette. */
        for (const dir of (cfg.bust === 'full' ? [] : [-1, 1])) {
          const bx = cx + dir * 3.9, by = SHOULDER_Y + 6.2;
          const hl = g.createRadialGradient(bx - 1, by - 1.4, 0.3, bx, by, 4.4);
          hl.addColorStop(0, 'rgba(255,240,224,.26)'); hl.addColorStop(1, 'rgba(255,240,224,0)');
          g.fillStyle = hl;
          g.beginPath(); g.arc(bx, by, 4.4, 0, WS.TAU); g.fill();
          g.strokeStyle = 'rgba(0,0,0,.26)'; g.lineWidth = 1;
          g.beginPath();
          g.moveTo(bx + dir * 3.4, by + 2.2);
          g.quadraticCurveTo(bx + dir * 1.2, by + 4.4, bx - dir * 2.2, by + 3.6);
          g.stroke();
        }
        // and the waist drawn in above the belt
        for (const dir of [-1, 1]) {
          const sh = g.createLinearGradient(cx + dir * b.waist, 0, cx + dir * (b.waist - 3), 0);
          sh.addColorStop(0, 'rgba(0,0,0,.28)'); sh.addColorStop(1, 'rgba(0,0,0,0)');
          g.fillStyle = sh;
          g.fillRect(WS.min(cx + dir * b.waist, cx + dir * (b.waist - 3)), SHOULDER_Y + 10, 3, WAIST_Y - SHOULDER_Y - 10);
        }
      } else {
        // A chest: the pectoral shelf and the line down the sternum.
        g.strokeStyle = 'rgba(0,0,0,.26)'; g.lineWidth = 1;
        for (const dir of [-1, 1]) {
          g.beginPath();
          g.moveTo(cx + dir * (b.sh - 3), SHOULDER_Y + 6.5);
          g.quadraticCurveTo(cx + dir * 4.5, SHOULDER_Y + 9.6, cx + dir * 0.6, SHOULDER_Y + 8.2);
          g.stroke();
        }
        g.globalAlpha = 0.8; g.lineWidth = 0.7;
        g.beginPath(); g.moveTo(cx, SHOULDER_Y + 2); g.lineTo(cx, SHOULDER_Y + 8); g.stroke();
      }
      g.restore();
    }
    if (cfg.bust === 'full') {
      /* A full figure under the robe: two rounded forms in the robe's own
         cloth, lit on the upper curve, shadowed beneath where the cloth
         falls away, with a fold between them. Drawn as shapes rather than
         shading so it reads as volume at play size, and set just wide of
         the torso so it is in the outline too. */
      const by = SHOULDER_Y + 8.4, rx = 5.9, ry = 4.9;
      for (const dir of [-1, 1]) {
        const bx = cx + dir * 4.9;
        g.save();
        g.fillStyle = 'rgba(0,0,0,.32)';
        g.beginPath(); g.ellipse(bx + dir * 0.3, by + 1.4, rx, ry * 0.9, 0, 0, WS.TAU); g.fill();
        lit(g, C.bodyRamp, bx - rx, by - ry, bx + rx, by + ry);
        g.beginPath(); g.ellipse(bx, by, rx, ry, dir * 0.12, 0, WS.TAU); g.fill();
        g.beginPath(); g.ellipse(bx, by, rx, ry, dir * 0.12, 0, WS.TAU); g.clip();
        const hl = g.createRadialGradient(bx - 1.6, by - 2, 0.4, bx - 0.6, by - 1, rx);
        hl.addColorStop(0, 'rgba(255,244,228,.42)'); hl.addColorStop(1, 'rgba(255,244,228,0)');
        g.fillStyle = hl; g.fillRect(bx - rx, by - ry, rx * 2, ry * 2);
        const un = g.createLinearGradient(0, by + 0.5, 0, by + ry);
        un.addColorStop(0, 'rgba(0,0,0,0)'); un.addColorStop(1, 'rgba(0,0,0,.4)');
        g.fillStyle = un; g.fillRect(bx - rx, by, rx * 2, ry);
        g.restore();
        g.strokeStyle = C.bodyRamp.line; g.globalAlpha = 0.55; g.lineWidth = 0.7;
        g.beginPath(); g.ellipse(bx, by, rx, ry, dir * 0.12, WS.PI * 0.1, WS.PI * 0.9); g.stroke();
        g.globalAlpha = 1;
      }
    ...
    if (cfg.bodice) {
      /* A milkmaid's blouse and bodice. Linen from the collar to the bust,
         gathered at the neckline, and a laced bodice from there to the
         waist - the dairy's working dress, and the one torso in the cast
         laced up the front. */
      g.save();
      g.beginPath();
      g.moveTo(torso[0][0], torso[0][1]);
      for (let i = 1; i < torso.length; i++) g.lineTo(torso[i][0], torso[i][1]);
      g.closePath(); g.clip();
      const linen = ramp([0.93, 0.91, 0.86]);
      lit(g, linen, cx - b.sh, SHOULDER_Y - 2, cx + b.sh, SHOULDER_Y + 10);
      g.fillRect(cx - b.sh - 1, SHOULDER_Y - 3, b.sh * 2 + 2, 13);
      g.strokeStyle = linen.line; g.globalAlpha = 0.45; g.lineWidth = 0.6;
      for (let k = -3; k <= 3; k++) {
        g.beginPath(); g.moveTo(cx + k * 3, SHOULDER_Y - 1); g.lineTo(cx + k * 3.4, SHOULDER_Y + 9); g.stroke();
      }
      g.globalAlpha = 1;
      const bod = ramp([0.46, 0.22, 0.16], 'leather');
      const top = SHOULDER_Y + 8;
      const bp = [[cx - b.sh + 1, top + 1], [cx - 3, top - 1.5], [cx, top + 1.2], [cx + 3, top - 1.5], [cx + b.sh - 1, top + 1],
        [cx + b.waist + 0.5, WAIST_Y], [cx - b.waist - 0.5, WAIST_Y]];
      panel(g, bp, bod);
      g.restore();
      // the lacing: eyelets down each side of the front and the cord crossing between them
      g.strokeStyle = '#efe6d2'; g.lineWidth = 0.6;
      g.beginPath();
      for (let k = 0; k < 3; k++) {
        const y = top + 1.6 + k * 2.6;
        g.moveTo(cx - 1.9, y); g.lineTo(cx + 1.9, y + 2.6);
        g.moveTo(cx + 1.9, y); g.lineTo(cx - 1.9, y + 2.6);
      }
      g.stroke();
      g.fillStyle = GOLD.key;
      for (let k = 0; k <= 3; k++) {
        for (const dir of [-1, 1]) { g.beginPath(); g.arc(cx + dir * 2.1, top + 1.6 + k * 2.6, 0.45, 0, WS.TAU); g.fill(); }
      }
    }
    if (cfg.gi) {
      /* A monk's gi: cloth wrapped left over right and open to the sternum,
         not padded armour. Plain weave with two long folds where the wrap
         pulls across, a bare chest in the V, and the lapels as two bands of
         his colour. The only survivor who shows skin below the neck. */
      g.save();
      g.beginPath();
      g.moveTo(torso[0][0], torso[0][1]);
      for (let i = 1; i < torso.length; i++) g.lineTo(torso[i][0], torso[i][1]);
      ...
      for (const dir of [-1, 1]) {
        g.beginPath();
        g.moveTo(cx + dir * (b.sh - 1), SHOULDER_Y + 1);
        g.quadraticCurveTo(cx + dir * (b.sh - 5), SHOULDER_Y + 10, cx - dir * 2, WAIST_Y - 3);
        g.stroke();
      }
      g.restore();
      const vee = [[cx - 5.4, SHOULDER_Y - 2.8], [cx + 5.4, SHOULDER_Y - 2.8], [cx + 0.8, SHOULDER_Y + 12.5], [cx - 1, SHOULDER_Y + 12.5]];
      panel(g, vee, cfg.skin || SKIN);
      g.save();
      g.strokeStyle = 'rgba(60,34,22,.45)'; g.lineWidth = 0.8;
      for (const dir of [-1, 1]) {
        g.beginPath(); g.moveTo(cx + dir * 3.6, SHOULDER_Y + 3.2); g.quadraticCurveTo(cx + dir * 1.8, SHOULDER_Y + 5.4, cx, SHOULDER_Y + 5); g.stroke();
      }
      g.beginPath(); g.moveTo(cx, SHOULDER_Y + 5.4); g.lineTo(cx, SHOULDER_Y + 10.5); g.stroke();
      g.restore();
      // the under lapel, then the one that wraps over it
      panel(g, [[cx + 5.4, SHOULDER_Y - 3], [cx + 8.2, SHOULDER_Y - 2.6], [cx + 0.6, WAIST_Y - 3.6], [cx - 2.2, WAIST_Y - 3.8]], C.accentRamp);
      panel(g, [[cx - 8.2, SHOULDER_Y - 2.6], [cx - 5.4, SHOULDER_Y - 3], [cx + 3.4, WAIST_Y - 3.8], [cx + 0.4, WAIST_Y - 3.4]], C.accentRamp);
      g.save();
      g.strokeStyle = C.accentRamp.key; g.globalAlpha = 0.6; g.lineWidth = 0.5;
      g.beginPath(); g.moveTo(cx - 7, SHOULDER_Y - 2.2); g.lineTo(cx + 1.6, WAIST_Y - 3.8); g.stroke();
      g.restore();
    }
    if (cfg.pauldrons) {
    ...
    if (cfg.stole) {
      ...
      for (const dir of [-1, 1]) {
        const x = cx + dir * 6.5;
        if (cfg.bust === 'full') {
          // lying over the curve: out and round the chest, then straight down
          const path = [[dir * 1.2, SHOULDER_Y - 1], [dir * 3.4, SHOULDER_Y + 4.5], [dir * 4.6, SHOULDER_Y + 9],
            [dir * 3.4, SHOULDER_Y + 13.5], [dir * 1.2, WAIST_Y], [dir * 1.2, WAIST_Y + 9]];
          const L = path.map(([dx, y]) => [x + dx - 1.6, y]), R = path.map(([dx, y]) => [x + dx + 1.6, y]).reverse();
          panel(g, L.concat(R), C.trimRamp);
        } else
    ...
    if (cfg.bandolier) {
      /* A bandolier of throwing knives, shoulder to hip. The blades in his
         hands are the ones you see coming; these are the rest of the
         argument. Hilts up out of their loops, so the chest reads as a row
         of steel at any size. */
      const p0 = [cx + b.sh - 3, SHOULDER_Y - 1.5], p1 = [cx - b.waist - 1.5, WAIST_Y - 2.5];
      panel(g, [[p0[0] + 2.6, p0[1] - 0.8], [p0[0] - 2.6, p0[1] - 0.2],
        [p1[0] - 1, p1[1] + 0.6], [p1[0] + 4.2, p1[1] + 0.4]], ramp([0.2, 0.16, 0.13], 'leather'));
      for (let k = 0; k < 4; k++) {
        const t = 0.16 + k * 0.2;
        const x = p0[0] + (p1[0] - p0[0]) * t + 0.8, y = p0[1] + (p1[1] - p0[1]) * t;
        g.save(); g.translate(x, y); g.rotate(-0.62);
        panel(g, [[-1.1, -5.4], [1.1, -5.4], [1.3, 0.6], [-1.3, 0.6]], ramp([0.30, 0.10, 0.10], 'leather'));
        panel(g, [[-2, 0.2], [2, 0.2], [1.8, 1.6], [-1.8, 1.6]], STEEL);
        g.fillStyle = STEEL.key;
        g.beginPath(); g.arc(0, -5.8, 0.9, 0, WS.TAU); g.fill();
        g.restore();
      }
      rivets(g, GOLD, p0[0] - 1, p0[1] + 2, p1[0] + 2, p1[1] - 1, 3);
    }
    if (cfg.strap) {
  ...
  const WEAPONS = {
    ...
    daggers(g, C, b) {
      ...
      for (const dir of [-1, 1]) {
        ...
        /* Long, and with a belly: a fighting knife is wider a third of the
           way down and sweeps to its point, where a straight wedge is a
           letter-opener. A clip on the back edge gives it the bite. */
        panel(g, [[-2.5, 0], [2.5, 0], [3.1, -10], [2.2, -20], [0, -28], [-1.4, -23.5], [-2.2, -21], [-1.9, -10]], STEEL);
        ...
        g.beginPath(); g.moveTo(0, -2.5); g.lineTo(0.2, -21); g.stroke();
        g.strokeStyle = 'rgba(255,255,255,.55)'; g.lineWidth = 0.7;
        g.beginPath(); g.moveTo(1.5, -3); g.lineTo(2.1, -12); g.lineTo(1.4, -21); g.lineTo(0, -26.4);
        g.stroke();
        ...
        // bound in the same red as the mask
        panel(g, [[-2.3, 1.6], [2.3, 1.6], [2, 6.4], [-2, 6.4]], ramp([0.5, 0.1, 0.1], 'leather'));
        g.save();
  ...
  const CAST = {
    ...
    priest: { build: 'slim', head: 'bare', robe: true, cloak: 'short', halo: true, fem: true,
      weapon: 'censer', stole: true, hair: ramp([0.55, 0.46, 0.33]), locks: true, bust: 'full',
      brand: 'priest', crownPoints: 13, veil: true },
    rogue: { build: 'slim', head: 'hood', cloak: 'cut', weapon: 'daggers', hoodFace: true,
      hood: ramp([0.2, 0.19, 0.21]), bandolier: true, downRoom: 0.86,
     ...
     * put her inside 83% of the mage's outline and 81% of the warlock's -
     * three survivors sharing one shape. Legs and a ragged hem take her out of
     * that cluster without touching anything else about her.
     * The moustache is Vonnra's own mark in the same sense the crown is
     * graveblade's - it is drawn nowhere else in the cast. */
    shaman: { build: 'normal', head: 'bare', cloak: 'tattered', weapon: 'totem', fem: true,
      mantle: true, warpaint: true, armwraps: true, hair: ramp([0.22, 0.24, 0.30]), longHair: 14, braids: 'two',
    ...
    ruinseeker: { build: 'normal', head: 'bare', horns: true, cloak: 'tattered',
      weapon: 'glaives', blindfold: true, sigils: true, bracer: true, legwraps: true, downRoom: 0.93,
    ...
    druid: { build: 'heavy', head: 'bovine', robe: true, robeFlare: 3, cloak: 'short', weapon: 'crook', fem: true,
      bell: true, apron: true, bodice: true, skin: ramp([0.90, 0.86, 0.80], 'skin'), skinBase: [0.90, 0.86, 0.80],
    ...
    monk: { build: 'slim', head: 'bare', bald: true, sash: true, weapon: 'mala', wraps: true,
      gi: true, goatee: ramp([0.80, 0.79, 0.76]),
  ...
  function colours(tint, demon, eyeOverride) {
    ...
    return {
      ...
      /* The iris of an open eye: the survivor's colour grounded toward a
         warm brown, so a white-tinted priest still has an eye with a colour
         in it and a saturated one does not glare. */
      iris: demon ? '#b8e84a' : WS.hex(WS.mix(t, [0.36, 0.25, 0.17], 0.45)),
      irisRing: demon ? '#2a3a10' : WS.hex(WS.mix(t, [0.1, 0.07, 0.05], 0.72)),
    };
  }
  ...
  function veil(g, C, front, hr) {
    ...
    if (!front) {
      ...
      g.restore();
      /* The inside of the veil is her hair, not more linen. Seen past a
         narrow chin, white linen under the jaw read as a white beard. */
      if (hr) {
        // her hair down the inside of the veil, either side...
        lit(g, hr, cx - 11, HEAD_CY - 6, cx + 11, SHOULDER_Y + 4);
        g.beginPath();
        g.moveTo(cx - 10.5, HEAD_CY - 6);
        g.quadraticCurveTo(cx - 12.5, HEAD_CY + 10, cx - 9, SHOULDER_Y + 4);
        g.lineTo(cx + 9, SHOULDER_Y + 4);
        g.quadraticCurveTo(cx + 12.5, HEAD_CY + 10, cx + 10.5, HEAD_CY - 6);
        g.closePath(); g.fill();
        // ...and her neck between them, in the shadow of the jaw
        const nk = g.createLinearGradient(0, HEAD_CY + 8, 0, SHOULDER_Y + 3);
        nk.addColorStop(0, 'rgb(112,78,58)'); nk.addColorStop(0.45, 'rgb(176,128,98)'); nk.addColorStop(1, 'rgb(196,148,114)');
        g.fillStyle = nk;
        g.beginPath();
        g.moveTo(cx - 4.4, HEAD_CY + 6);
        g.quadraticCurveTo(cx - 5.4, SHOULDER_Y - 3, cx - 8.2, SHOULDER_Y + 4);
        g.lineTo(cx + 8.2, SHOULDER_Y + 4);
        g.quadraticCurveTo(cx + 5.4, SHOULDER_Y - 3, cx + 4.4, HEAD_CY + 6);
        g.closePath(); g.fill();
        // a fine chain with the Morning Light's sun on it
        g.strokeStyle = GOLD.key; g.lineWidth = 0.55;
        g.beginPath(); g.moveTo(cx - 5.8, NECK_Y - 1); g.quadraticCurveTo(cx, SHOULDER_Y + 2.6, cx + 5.8, NECK_Y - 1); g.stroke();
        g.fillStyle = GOLD.key;
        g.beginPath(); g.arc(cx, SHOULDER_Y + 1.6, 1.25, 0, WS.TAU); g.fill();
        glow(g, cx, SHOULDER_Y + 1.6, 3, C.accent, 0.5);
      }
    } else {
  ...
  function elephant(g, x, y, w, C) {
    ...
    g.fillStyle = far.shade;
    g.beginPath(); g.ellipse(x + 8.6, y + 2.2, 0.8, 1.1, 0, 0, WS.TAU); g.fill();
    // body: round, a little higher behind than in front
    lit(g, pink, x - 5, y - 7, x + 7.5, y + 2.5);
    g.beginPath(); g.ellipse(x + 1.6, y - 2, 6.2, 4.5, -0.08, 0, WS.TAU); g.fill();
    finish(g, pink, x - 4.6, y - 6.6, x + 7.8, y + 2.6);
    // a saddle cloth in her own colour, trimmed in gold
    panel(g, [[x - 0.8, y - 6.2], [x + 5.6, y - 6], [x + 6.2, y - 1.6], [x + 2.6, y - 0.4], [x - 1.2, y - 1.8]], C.accentRamp);
    g.strokeStyle = GOLD.key; g.lineWidth = 0.55; g.globalAlpha = 0.9;
    ...
    g.beginPath(); g.ellipse(x - 4.4, y - 6, 3.8, 3.5, 0, 0, WS.TAU); g.fill();
    // the trunk: down off the face, then up in a curl - it is trumpeting
    const P = (t) => {
      const u = 1 - t;
      const p0 = [x - 7.4, y - 5], c1 = [x - 10.8, y - 2.4], p2 = [x - 10.6, y - 10.6];
      return [u * u * p0[0] + 2 * u * t * c1[0] + t * t * p2[0], u * u * p0[1] + 2 * u * t * c1[1] + t * t * p2[1]];
    };
    const W = (t) => 1.45 * (1 - t) + 0.55;
    lit(g, pink, x - 11, y - 11, x - 7, y - 3);
    sweptPath(g, P, W, 14); g.fill();
    g.save(); g.strokeStyle = pink.line; g.globalAlpha = 0.5; g.lineWidth = 0.35;
    for (let i = 1; i < 6; i++) {
      const [px, py] = P(i / 6);
      g.beginPath(); g.moveTo(px - 0.9, py); g.lineTo(px + 0.9, py - 0.2); g.stroke();
    }
    g.restore();
    g.fillStyle = pink.line;
    g.beginPath(); g.ellipse(x - 10.6, y - 10.8, 0.7, 0.45, 0, 0, WS.TAU); g.fill();
    // the ear, big and soft, over the neck
    const ear = ramp([0.96, 0.70, 0.78], 'skin');
    lit(g, ear, x - 3.6, y - 9.6, x + 1.8, y - 1.8);
    g.beginPath();
    g.moveTo(x - 2.6, y - 8.6);
    g.bezierCurveTo(x + 1.6, y - 10.4, x + 2.8, y - 4.6, x + 0.4, y - 2.2);
    g.bezierCurveTo(x - 1.4, y - 1, x - 3.2, y - 3.6, x - 2.6, y - 8.6);
    g.closePath(); g.fill();
    g.fillStyle = 'rgba(214,110,140,.45)';
    g.beginPath(); g.ellipse(x - 0.8, y - 5.6, 1.5, 2.4, 0.2, 0, WS.TAU); g.fill();
    g.strokeStyle = pink.line; g.lineWidth = 0.5; g.globalAlpha = 0.7;
    g.beginPath();
    g.moveTo(x - 2.6, y - 8.6);
    g.bezierCurveTo(x + 1.6, y - 10.4, x + 2.8, y - 4.6, x + 0.4, y - 2.2);
    g.stroke();
    g.globalAlpha = 1;
    // an eye with a catchlight and two lashes, and a cheek
    g.fillStyle = '#1c1014';
    g.beginPath(); g.ellipse(x - 5.4, y - 6.9, 0.75, 0.9, 0, 0, WS.TAU); g.fill();
    g.fillStyle = '#fff';
    g.beginPath(); g.arc(x - 5.6, y - 7.2, 0.28, 0, WS.TAU); g.fill();
    g.strokeStyle = '#1c1014'; g.lineWidth = 0.4;
    for (const [dx, dy] of [[-0.2, -1.6], [0.6, -1.4]]) {
      g.beginPath(); g.moveTo(x - 5.4 + dx * 0.3, y - 7.7); g.lineTo(x - 5.4 + dx, y - 6.9 + dy); g.stroke();
    }
    g.fillStyle = 'rgba(230,110,130,.45)';
    g.beginPath(); g.arc(x - 6.4, y - 5.2, 0.9, 0, WS.TAU); g.fill();
  }
  ...
  /* ------------------------------------------------------------ hair --- */
  /** Hair that falls past the shoulders, behind them. Drawn before the body,
   *  so the torso and the head cover all but the fall at either side. */
  function longHair(g, cfg, len) {
    const cx = 50, hr = cfg.hair || DARKCLOTH;
    const bot = SHOULDER_Y + len;
    lit(g, hr, cx - 15, HEAD_CY - HEAD_RY, cx + 15, bot);
    g.beginPath();
    g.moveTo(cx - HEAD_RX - 0.5, HEAD_CY - 4);
    g.bezierCurveTo(cx - HEAD_RX - 1, HEAD_CY - HEAD_RY * 1.4, cx + HEAD_RX + 1, HEAD_CY - HEAD_RY * 1.4,
    ...
    g.closePath(); g.fill();
    finish(g, hr, cx - HEAD_RX - 4, HEAD_CY - HEAD_RY * 1.2, cx + HEAD_RX + 4, bot + 1);
    // strands, the way it falls
    g.save();
    g.strokeStyle = hr.line; g.globalAlpha = 0.45; g.lineWidth = 0.6;
    for (const dir of [-1, 1]) {
      for (const o of [0.5, 2.2]) {
        g.beginPath();
        g.moveTo(cx + dir * (HEAD_RX - 1 + o), HEAD_CY + 2);
        g.quadraticCurveTo(cx + dir * (HEAD_RX + 2 + o), HEAD_CY + 14, cx + dir * (HEAD_RX + o * 0.6), bot - 1);
        g.stroke();
      }
    }
    g.restore();
  ...
  /** A plait: a chain of lobes along a curve, each tilted the other way from
   *  the last, tied off at the end with a tuft. `beads` threads one of the
   *  survivor's colour on every other lobe. */
  function braid(g, hr, x0, y0, cxp, cyp, x1, y1, n, beads) {
    const P = (t) => {
      const u = 1 - t;
      return [u * u * x0 + 2 * u * t * cxp + t * t * x1, u * u * y0 + 2 * u * t * cyp + t * t * y1];
    };
    for (let i = 0; i < n; i++) {
      const t = (i + 0.5) / n;
      const [x, y] = P(t);
      const r = 2.5 - t * 0.7;
      lit(g, hr, x - r, y - r, x + r, y + r);
      g.beginPath(); g.ellipse(x, y, r, r * 0.78, (i % 2 ? 0.55 : -0.55), 0, WS.TAU); g.fill();
      g.strokeStyle = hr.line; g.globalAlpha = 0.55; g.lineWidth = 0.5;
      g.beginPath(); g.ellipse(x, y, r, r * 0.78, (i % 2 ? 0.55 : -0.55), 0, WS.TAU); g.stroke();
      g.globalAlpha = 1;
      if (beads && i % 2) {
        g.fillStyle = beads.core;
        g.beginPath(); g.arc(x, y, 1.15, 0, WS.TAU); g.fill();
        g.fillStyle = 'rgba(255,255,255,.6)';
        g.beginPath(); g.arc(x - 0.35, y - 0.35, 0.4, 0, WS.TAU); g.fill();
      }
    }
    // the tie and the tuft
    const [ex, ey] = P(1);
    panel(g, [[ex - 1.6, ey - 1.2], [ex + 1.6, ey - 1.2], [ex + 1.4, ey + 0.6], [ex - 1.4, ey + 0.6]], beads || LEATHER);
    panel(g, [[ex - 1.4, ey + 0.4], [ex + 1.4, ey + 0.4], [ex + 2.2, ey + 4.4], [ex, ey + 3.2], [ex - 2, ey + 4.6]], hr);
  }

  /** Loose locks at either side of the face, from under a veil or a band. */
  function faceLocks(g, hr) {
    const cx = 50;
    for (const dir of [-1, 1]) {
      lit(g, hr, cx + dir * 8, HEAD_CY - 6, cx + dir * 12, SHOULDER_Y + 3);
      g.beginPath();
      g.moveTo(cx + dir * 7.4, HEAD_CY - 6);
      g.bezierCurveTo(cx + dir * 10.6, HEAD_CY + 1, cx + dir * 8.6, HEAD_CY + 8, cx + dir * 11.2, SHOULDER_Y + 3);
      g.bezierCurveTo(cx + dir * 12.4, HEAD_CY + 8, cx + dir * 13.2, HEAD_CY, cx + dir * 11.2, HEAD_CY - 7);
      g.closePath(); g.fill();
      g.strokeStyle = hr.line; g.globalAlpha = 0.5; g.lineWidth = 0.5;
      g.beginPath();
      g.moveTo(cx + dir * 9.6, HEAD_CY - 4);
      g.bezierCurveTo(cx + dir * 11.6, HEAD_CY + 2, cx + dir * 10, HEAD_CY + 8, cx + dir * 11.6, SHOULDER_Y + 1);
      g.stroke();
      g.globalAlpha = 1;
    }
  }
  ...
  function figure(g, cfg, C, w) {
    let b = BUILDS[cfg.build] || BUILDS.normal;
    /* The frame under the clothes. Narrower shoulders, a drawn-in waist and
       more hip for the women; for the men who are not already built like a
       door, a little more shoulder over the same waist - the taper is what
       says it, not the size. */
    if (cfg.fem) {
      b = Object.assign({}, b, { sh: b.sh - 1.4, waist: b.waist - 1.3, hip: b.hip + 1.2, arm: b.arm - 0.3 });
    } else if (cfg.build !== 'heavy') {
      b = Object.assign({}, b, { sh: b.sh + 1.2, arm: b.arm + 0.3 });
    }
    ...
    if (cfg.longHair) {
      g.save(); g.translate(0, -lift);
      longHair(g, cfg, cfg.longHair);
      g.restore();
    }
    drawBody(g, cfg, C, b, w, lift);
    if (cfg.braids) {
      g.save(); g.translate(0, -lift);
      const hr = cfg.hair || DARKCLOTH;
      if (cfg.braids === 'one') {
        // one plait over the near shoulder, the way a ranger keeps it out of the string
        braid(g, hr, 58.5, HEAD_CY + 4, 62, SHOULDER_Y + 2, 59, WAIST_Y - 8, 7, null);
      } else {
        for (const dir of [-1, 1]) {
          braid(g, hr, 50 + dir * 9.6, HEAD_CY + 4, 50 + dir * 12.4, SHOULDER_Y, 50 + dir * 9.4, SHOULDER_Y + 13, 6, C.accentRamp);
        }
      }
      g.restore();
    }
    ...
    if (cfg.veil) veil(g, C, false, cfg.hair);
    drawHead(g, cfg, C);
    if (cfg.locks) faceLocks(g, cfg.hair || DARKCLOTH);
  ...
  const Hero = {
    ...
    draw(g, size, id, tint, demon, phase, pose, rank) {
      ...
      let w = posed ? POSES[pose.kind](pose.k)
        : (phase === undefined ? STILL : stride(phase));
      /* A survivor who carries something long enough to leave the tile as
         they go over asks for a little more of the shrink, in proportion to
         how far down they are (CAST.downRoom). */
      if (posed && pose.kind === 'down' && cfg.downRoom) {
        const k = (1 - w.shrink) / 0.26;
        w = Object.assign({}, w, { shrink: w.shrink * (1 - (1 - cfg.downRoom) * k) });
      }

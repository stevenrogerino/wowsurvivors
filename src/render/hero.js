...
(function (WS) {
  ...
  function drawBody(g, cfg, C, b) {
    ...
    if (cfg.backShield) {
      /* A shield slung on his back, drawn before everything so it reads as
       * being behind him. The warrior's outline was two pauldrons and a haft;
       * a disc breaking the line above one shoulder is the third shape. */
      const sx2 = cx - b.sh - 2;
      lit(g, ramp([0.46, 0.42, 0.36]), sx2 - 12, SHOULDER_Y - 14, sx2 + 8, SHOULDER_Y + 8);
      g.beginPath(); g.ellipse(sx2 - 2, SHOULDER_Y - 3, 12.5, 13.5, -0.2, 0, WS.TAU); g.fill();
      g.strokeStyle = GOLD.shade; g.lineWidth = 1.4;
      g.beginPath(); g.ellipse(sx2 - 2, SHOULDER_Y - 3, 8, 8.8, -0.2, 0, WS.TAU); g.stroke();
      g.fillStyle = STEEL.core;
      g.beginPath(); g.arc(sx2 - 2, SHOULDER_Y - 3, 3, 0, WS.TAU); g.fill();
    }
    if (cfg.legwraps) {
   ...
   *   warrior     horns off the helm, heavy at the root and curling up, and a
   *               shield slung on his back to break the shoulder line
   ...
   *   graveblade  a dead man's face - pallid, hollow-cheeked, sockets with no
   *               eyes in them and a cold light coming out anyway - and a rune
   *               channel burning the length of his blade. He wore a helm like
   *               the other two heavies, which hid the one fact about him that
   *               matters
  ...
  const CAST = {
    ...
    warrior: { build: 'heavy', head: 'helm', pauldrons: 2.1, weapon: 'axe',
      helmHorns: true, sash: true, backShield: true },
    ...
    graveblade: { build: 'heavy', head: 'undead', undead: true, pauldrons: 1.7,
      cloak: 'tattered', weapon: 'greatsword', chains: true },
  ...
  function colours(tint, demon) {
    ...
    return {
      ...
      eyeRgb: demon ? [0.85, 1, 0.42] : WS.mix(t, [1, 1, 1], 0.72),
      ...
      channelRgb: demon ? [0.7, 1, 0.35] : WS.mix(t, [1, 1, 1], 0.30),

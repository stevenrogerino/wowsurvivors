...
(function (WS) {
  ...
  P.launchBolt = function (x, y, vx, vy, spec) {
    ...
    b.rank = spec.rank || 1;
    b.lastHx = null; b.lastHy = null;    // where it last struck anything
  ...
  P.spawnBeam = function (x1, y1, x2, y2, width, colour, life) {
    ...
    b.art = null; b.arc = false;   // a chain link sets arc: drawn as lightning
  ...
  function expire(b) {
    if (b.endBurst) {
      /* Only herds carry a burst (Moonlit Herd, The Wild Hunt), and a herd
         runs through the crowd and out the far side: the end of the run is
         open ground. The burst goes off where the beast last trampled
         something, which is where there is anything left to hit. */
      const bx = b.lastHx === null ? b.x : b.lastHx, by = b.lastHx === null ? b.y : b.lastHy;
      WS.Enemy.damageArea(bx, by, b.endBurst, b.damage * 0.9, null, 18, b.source);
      WS.FX.flash(bx, by, b.endBurst, b.colour, 0.26, 6, 'physical');
    }
    if (b.endZone) thicket(b);
  }

  /* Bramble Run's thicket. It used to grow where the herd's run ENDED - but
     a spirit beast tramples straight through the crowd and keeps running, so
     the end of the run was nearly always open ground past it. Measured over
     90s of play at 10:00: 30% of thickets never touched a creature and half
     of all their ticks struck nothing - green ground that sometimes did
     something, which is what a tester noticed. It grows now where the beast
     last trampled something, which is where the crowd is; a beast that met
     nobody grows nothing, because there is nothing there to hold. */
  function thicket(b) {
    if (b.lastHx === null) return;
    const z = P.spawnZone(b.lastHx, b.lastHy, 60, b.damage * 0.5, 2.5, 0.5, [0.46, 0.72, 0.30], b.source);
  ...
  P.update = function (dt) {
    ...
    while (i < this.bolts.count) {
      ...
      if (b.x < -OFF || b.x > W + OFF || b.y < -OFF || b.y > H + OFF) {
        if (b.endZone) thicket(b);      // a run off the field still seeds where it struck
        this.bolts.releaseAt(i); continue;
      }

...
(function (WS) {
  ...
  Audio.init = function () {
    ...
    /* A ROOM. Nothing in the game had one: every voice was bone dry, so a
     * bell stopped the instant it stopped and a boss's horn sounded like it
     * was playing inside the listener's head. One generated impulse - a
     * couple of seconds of noise that darkens as it decays, the way air eats
     * the top of a real tail - feeds two convolvers, one behind the effects
     * and one behind the score, so each still answers its own volume slider
     * and the music switch still silences everything the music made. Sends,
     * not inserts: a voice asks for as much room as it wants and the dry
     * signal is untouched. */
    const ir = makeRoom(this.ctx, 2.6);
    const room = (dest, level) => {
      const inp = this.ctx.createGain();
      const conv = this.ctx.createConvolver();
      conv.buffer = ir;
      const out = this.ctx.createGain();
      out.gain.value = level;
      inp.connect(conv); conv.connect(out); out.connect(dest);
      return inp;
    };
  ...
  /** The room's impulse: stereo noise that decays over `secs`, losing its
   *  top faster than its bottom, with a short gap before the first
   *  reflection. The two channels are drawn separately so the tail is wide. */
  function makeRoom(ctx, secs) {
    const rate = ctx.sampleRate, len = WS.floor(rate * secs), pre = WS.floor(rate * 0.014);
    ...
    for (let ch = 0; ch < 2; ch++) {
      ...
      for (let i = pre; i < len; i++) {
        ...
        // Darkening: the one-pole's coefficient falls as the tail ages.
        const a = 0.55 - 0.47 * k;
        lp += a * ((Math.random() * 2 - 1) - lp);
        d[i] = lp * Math.pow(1 - k, 2.2) * 0.6;
      }
    }
    return buf;
  }

  /** How much of a voice goes to the room. `music` picks the score's room. */
  function send(node, amt, music) {
    const room = music ? Audio.musicRoom : Audio.sfxRoom;
    if (!amt || !room) return;
    const g = Audio.ctx.createGain();
    g.gain.value = amt;
    node.connect(g); g.connect(room);
  }
  ...
  function tone(opts) {
    ...
    send(gain, opts.verb === undefined ? Audio.verbFor : opts.verb);
  ...
  /** The seven schools, as sound. */
  const CAST = {
    // A lick of flame: breath rising through a warm band, and a low push.
    fire() {
      noise({ filter: 'bandpass', freq: 700, to: vary(2200, 0.2), q: 1.1, attack: 0.03, decay: vary(0.16, 0.25), gain: 0.12 });
      tone({ type: 'sine', freq: 140, to: 90, decay: 0.12, gain: 0.05 });
    },
    // Glass: two high, pure partials that ring a moment, and a crystal tick.
    frost() {
      tone({ type: 'sine', freq: vary(1760, 0.04), decay: vary(0.22, 0.2), gain: 0.035 });
      tone({ type: 'sine', freq: vary(2637, 0.04), decay: vary(0.16, 0.2), gain: 0.022, delay: 0.012 });
      noise({ filter: 'highpass', freq: 5200, q: 0.8, attack: 0.001, decay: 0.012, gain: 0.02 });
    },
    // A shimmer: a detuned pair sweeping up, the arcane's glassy fizz.
    arcane() {
      tone({ type: 'triangle', freq: 520, to: vary(880, 0.12), decay: vary(0.12, 0.25), gain: 0.07 });
      tone({ type: 'sine', freq: 1046, to: vary(1568, 0.1), decay: 0.10, gain: 0.03, delay: 0.02 });
    },
    // A small bell: a soft attack and a fifth above it.
    holy() {
      tone({ type: 'sine', freq: vary(880, 0.03), decay: vary(0.35, 0.2), gain: 0.05, attack: 0.008 });
      tone({ type: 'sine', freq: vary(1320, 0.03), decay: vary(0.28, 0.2), gain: 0.025, attack: 0.008 });
    },
    // Wood and sap: a plucked, woody knock.
    nature() {
      tone({ type: 'triangle', freq: vary(330, 0.08), to: 240, decay: vary(0.10, 0.2), gain: 0.08, filter: 'lowpass', cutoff: 1400 });
      noise({ filter: 'bandpass', freq: 1300, q: 2.4, attack: 0.001, decay: 0.02, gain: 0.03 });
    },
    // Something drawn in: a dark swoop falling away under a breath.
    shadow() {
      tone({ type: 'sawtooth', freq: 260, to: vary(110, 0.15), decay: vary(0.16, 0.2), gain: 0.05, filter: 'lowpass', cutoff: 700 });
      noise({ filter: 'bandpass', freq: 1400, to: 500, q: 1.0, attack: 0.02, decay: 0.14, gain: 0.03 });
    },
    // Steel through air: a quick swish.
    physical() {
      noise({ filter: 'bandpass', freq: 1800, to: vary(3400, 0.2), q: 1.6, attack: 0.012, decay: vary(0.08, 0.25), gain: 0.17 });
    },
  };
  ...
  function chime(freq, decay, gain, delay) {
    ...
    tone({ type: 'sine', freq: freq * 5.4, decay: decay * 0.25, gain: gain * 0.15, delay, attack: 0.002 });
  }
  ...
  const KITS = {
    /* The five kits below carry most of a run between them, so they are also
       ...
       of separate events. */
    /* A SPELL SOUNDS LIKE ITS SCHOOL. Every weapon in the game used to cast
       with one triangle blip, so a build of six weapons was one sound six
    ...
    coin() {
      ...
      chime(vary(2530, 0.03), 0.2, 0.035, 0.05);
    },
    potion() {
    ...
    chest() {
      ...
      for (let i = 0; i < 6; i++) {
        chime(vary(1500 + i * 170, 0.04), 0.2, 0.06, 0.06 + i * 0.05 + Math.random() * 0.02);
      }
    },
    level() {
      // A rising chime, each note a struck bell rather than a beep.
      const notes = [523.25, 659.25, 783.99, 1046.5];
      notes.forEach((f, i) => {
        chime(f, 0.9, 0.08, i * 0.07);
        tone({ type: 'triangle', freq: f, decay: 0.4, gain: 0.05, delay: i * 0.07 });
      });
    },
    evolve() {
      // A swelling chord that opens, and a shower of bells over the top.
      for (const f of [196, 293.66, 392, 587.33]) {
        tone({ type: 'sawtooth', freq: f, decay: 1.4, gain: 0.05, attack: 0.25, filter: 'lowpass', cutoff: 1800 });
      }
      const notes = [523.25, 659.25, 783.99, 1046.5, 1318.5, 1567.98];
      notes.forEach((f, i) => chime(f, 1.1, 0.06, 0.18 + i * 0.07));
    },
    boss() {
      // A horn in a big room, and a gong under it.
      tone({ type: 'sawtooth', freq: 82, to: 55, decay: 1.6, gain: 0.24, filter: 'lowpass', cutoff: 600, attack: 0.08 });
      tone({ type: 'sawtooth', freq: 123, to: 82, decay: 1.4, gain: 0.16, filter: 'lowpass', cutoff: 800, delay: 0.1, attack: 0.08 });
    ...
    victory() {
      // A fanfare in bells over a held major chord.
      for (const f of [261.63, 329.63, 392, 523.25]) {
        tone({ type: 'triangle', freq: f, decay: 2.4, gain: 0.06, attack: 0.2 });
      }
      const notes = [523.25, 659.25, 783.99, 1046.5, 1318.5, 1567.98];
      notes.forEach((f, i) => chime(f, 1.3, 0.08, i * 0.12));
    },

    /* ---- the finales: each machine and monster sounds like itself ---- */
    // The Candlecrawler going under, or coming up: earth moving.
    rumble() {
      tone({ type: 'sawtooth', freq: 48, to: 34, decay: 1.4, gain: 0.2, filter: 'lowpass', cutoff: 180, attack: 0.1 });
      noise({ filter: 'lowpass', freq: 260, to: 90, q: 0.7, attack: 0.15, decay: 1.2, gain: 0.14 });
      for (let i = 0; i < 5; i++) {
        noise({ filter: 'bandpass', freq: vary(900, 0.3), q: 3, attack: 0.002, decay: 0.03, gain: 0.03, delay: 0.2 + i * 0.17 });
      }
    },
    // The Galleon's guns: a report, the boom behind it, and the shot's whistle.
    cannon() {
      noise({ filter: 'lowpass', freq: 1800, to: 120, q: 0.6, attack: 0.002, decay: 0.5, gain: 0.26 });
      tone({ type: 'sine', freq: 72, to: 38, decay: 0.6, gain: 0.26, attack: 0.002 });
      tone({ type: 'sine', freq: 1500, to: 700, decay: 0.45, gain: 0.02, delay: 0.1, attack: 0.05 });
    },
    // One of Mordecai's lanterns breaking: glass, and something let out.
    glass() {
      for (let i = 0; i < 7; i++) {
        tone({ type: 'sine', freq: vary(2400 + i * 520, 0.15), decay: 0.12 + Math.random() * 0.18, gain: 0.03, delay: Math.random() * 0.08, attack: 0.001 });
      }
      noise({ filter: 'highpass', freq: 3000, q: 0.7, attack: 0.001, decay: 0.08, gain: 0.07 });
      tone({ type: 'sine', freq: 520, to: 180, decay: 1.2, gain: 0.05, delay: 0.1, attack: 0.3 });
    },
    // Electricity arcing across the Stormbreaker's pylons.
    zap() {
      tone({ type: 'sawtooth', freq: vary(90, 0.2), decay: 0.3, gain: 0.22, filter: 'bandpass', cutoff: 1600 });
      for (let i = 0; i < 4; i++) {
        tone({ type: 'square', freq: vary(1200, 0.4), to: vary(300, 0.3), decay: 0.05, gain: 0.08, delay: i * 0.05 + Math.random() * 0.02 });
      }
    },
    // The Stormbreaker's stamp: a pressure wave through the ground.
    shock() {
      tone({ type: 'sine', freq: 90, to: 30, decay: 0.9, gain: 0.3, attack: 0.002 });
    ...
    drill() {
      ...
      tone({ type: 'square', freq: 440, to: 660, decay: 1.0, gain: 0.025, filter: 'lowpass', cutoff: 2000, attack: 0.1 });
      noise({ filter: 'bandpass', freq: 500, to: 900, q: 2, attack: 0.1, decay: 1.0, gain: 0.06 });
    },
    // Ice breaking - Marrowfrost's wards, his shards.
    shatter() {
      for (let i = 0; i < 9; i++) {
        tone({ type: 'sine', freq: vary(3100 + i * 380, 0.2), decay: 0.08 + Math.random() * 0.25, gain: 0.03, delay: Math.random() * 0.12, attack: 0.001 });
      }
      tone({ type: 'sine', freq: 220, to: 110, decay: 0.5, gain: 0.08, attack: 0.002 });
      noise({ filter: 'bandpass', freq: 4200, q: 1.2, attack: 0.001, decay: 0.1, gain: 0.05 });
    },
    // The winter coming down: a deep swell and a choir of cold.
    winter() {
      for (const [f, d] of [[55, 0], [82.4, 6], [110, -6], [164.8, 9]]) {
        tone({ type: 'sawtooth', freq: f, decay: 3.2, gain: 0.05, attack: 1.0, filter: 'lowpass', cutoff: 900 });
        tone({ type: 'sine', freq: f * 4, decay: 2.8, gain: 0.02, attack: 1.2 });
        void d;
      }
    },
    ...
    /* A charge committing: two stabs of low brass, the second a step up, so
       it reads as a threat drawing breath rather than a machine beeping. */
    warn() {
      for (const [f, d] of [[196, 0], [220, 0.22]]) {
        tone({ type: 'sawtooth', freq: f, decay: 0.2, gain: 0.13, delay: d, attack: 0.02,
          filter: 'lowpass', cutoff: 1400 });
        tone({ type: 'sine', freq: f / 2, decay: 0.22, gain: 0.08, delay: d, attack: 0.01 });
      }
    },
  };
   ...
   *  hit with no room is a click, one with a lot is a smear - and the rare,
   *  important sounds get enough to hang in the air after they end. */
  const ROOM = {
   ...
   *  @param {string} [variant]  a flavour of the kit - a spell's school. Each
   *  variant keeps its own throttle, so a fire spell never swallows the frost
   *  one that fired in the same tick. */
  Audio.pitch = 1;
  Audio.verbFor = 0;
  Audio.variant = null;

  Audio.play = function (kit, x, variant) {
    ...
    const resetAt = WS.max(gap, lastGap[key] || 0) * 3;
    if (chatter && gap && (waiting[key] || sustainStart[key])
        && t - (lastPlayed[key] || 0) > resetAt) {
      waiting[key] = 0;
      sustainStart[key] = null;   // the streak broke; fatigue breaks with it
    }
    /* How much has been happening in this kit lately, 0 to 1. Everything below
       reads the run's density through this one number. */
    const heft = chatter ? WS.min(1, (waiting[key] || 0) / HEFT) : 0;
     ...
     * its ceiling. But heft is `waiting[key] / HEFT`, and waiting is zeroed
    ...
    let fatigue = 0;
    if (chatter && gap) {
      if (!sustainStart[key]) sustainStart[key] = t;
      fatigue = WS.clamp((t - sustainStart[key] - FATIGUE_GRACE) / FATIGUE_WINDOW, 0, 1);
    }
    if (gap) {
      ...
      if (lastPlayed[key] && t - lastPlayed[key] < widened) {
        if (chatter) waiting[key] = (waiting[key] || 0) + 1;   // remembered, not lost
        return;
      }
      lastPlayed[key] = t;
      lastGap[key] = widened;
    ...
    if (chatter) {
      ...
      /* What the last chatter voice was shaped into. Exposed because it is the
         ...
         look at it. */
      this.lastShape = { kit: key, heft, fatigue, gain: shape.gain.value, pitch: this.pitch };
    } else if (spread) {
      this.pitch = 1 + (Math.random() - 0.5) * 2 * spread;
    }
    this.voiceBus = placeAt(x, dest) || dest;
    this.verbFor = ROOM[kit] || 0;
    this.variant = variant || null;
    try { fn(); } catch (e) { /* audio graph exhausted */ }
    finally { this.voiceBus = null; this.pitch = 1; this.verbFor = 0; this.variant = null; }
  };
  ...
  const SCORES = {
    ...
    plains: {
      ...
      perc: [0, 3, 6, 9, 12], kit: 'shake', percGain: 1.9,
      prog: [0, 4, 2, 3],
    ...
    cursed: {
      ...
      prog: [0, 1, 0, 4],
      life: { rate: 0.32, voices: ['owl', 'owl', 'wail', 'crow', 'cricket'] },
    ...
    savannah: {
      /* A reed, not a saw. The Dustreach and Ambergrass both led on a
         sawtooth within a semitone of each other, and the weather beds were
         what told them apart; with those gone the lead has to. A square is
         hollow where a saw is bright - the sound of a pipe across a plain. */
      root: 123.47, scale: [0, 2, 5, 7, 10], wave: 'square', tempo: 1.4,
      leadCut: 5,
      pad: 'sine', padCut: 1500,
      perc: [0, 3, 6, 8, 11, 14], kit: 'skin', percGain: 1.6,
      prog: [0, 2, 3, 1],
      life: { rate: 0.52, voices: ['cicada', 'cicada', 'cicada', 'bird'] },
    ...
    glacier: {
      ...
      perc: [0, 8], kit: 'bell', percGain: 2.6,
      prog: [0, 0, 3, 4],
    ...
    eclipse: {
      ...
      prog: [0, 3, 2, 3],
      life: { rate: 0.30, voices: ['toll', 'toll', 'deep'] },
    ...
    vigil: {
      ...
      pad: 'sine', padCut: 560,
      perc: null, kit: null,
      life: { rate: 0.16, voices: ['cricket', 'cricket', 'owl'] },
      quiet: 0.66,
  ...
  const KIT = {
    ...
    /* A rattle - seeds in a gourd - rather than the highpassed hiss it was:
       a woody click and a short band of noise that has a top to it. */
    shake(bed, t0, v) {
      mNoise(bed, t0, { freq: 3800, to: 2800, decay: 0.11, gain: 0.06 * v, q: 1.6 });
      mTone(bed, t0, { wave: 'triangle', freq: 820, decay: 0.03, gain: 0.020 * v, attack: 0.002 });
    },
    skin(bed, t0, v) {
    ...
    bell(bed, t0, v) {
      mTone(bed, t0, { wave: 'square', freq: 1244.5, decay: 1.10, gain: 0.020 * v,
        cut: 2400, attack: 0.004, verb: 0.5 });
      mTone(bed, t0, { wave: 'square', freq: 1864.7, decay: 0.80, gain: 0.010 * v,
        cut: 3000, attack: 0.004, detune: 9, verb: 0.4 });
    },
    stone(bed, t0, v) {
  ...
  /* ---------------------------------------------------------------- life --
   * THE PLACES ARE MADE OF WHAT LIVES IN THEM. Each battlefield names a few
   * small voices and a rate, and the score's own clock calls one up now and
   * then, somewhere in the stereo field, with a little of the room on it:
   * crickets and frogs in the woods, crows over the Dustreach, owls and
   * something wailing in Mourneholt, cicadas on the plains, wind whistling
   * through ice and a chime in the Rimewaste, a shimmer in the Eclipse.
   * Everything is pitched and everything ends - there is no bed under any of
   * it - so nothing can settle into a hiss. The one noise-based voice, the
   * Dustreach's gust, is kept below 600Hz, where noise is wind and not air. */
  function lifeOut(bed, pan, verb) {
    const ctx = Audio.ctx;
    const g = ctx.createGain();
    g.gain.value = 1;
    let out = g;
    ...
    out.connect(bed);
    if (verb) send(g, verb, true);
    setTimeout(() => { try { g.disconnect(); if (out !== g) out.disconnect(); } catch (e) { /* gone */ } }, 9000);
    return g;
  }
  const LIFE = {
    cricket(o, t) {
      const f = 4000 + WS.random() * 900, chirps = 3 + WS.floor(WS.random() * 4);
      for (let c = 0; c < chirps; c++) {
        for (let k = 0; k < 4; k++) {
          mTone(o, t + c * 0.32 + k * 0.024, { wave: 'sine', freq: f, gain: 0.024, attack: 0.003, decay: 0.014 });
        }
      }
    },
    bird(o, t) {
      const n = 2 + WS.floor(WS.random() * 3), base = 2300 + WS.random() * 1100;
      for (let k = 0; k < n; k++) {
        const up = WS.random() < 0.5;
        mTone(o, t + k * 0.14, { wave: 'sine', freq: base * (up ? 0.85 : 1.15), to: base * (up ? 1.2 : 0.8),
          slide: 0.09, gain: 0.016, attack: 0.01, decay: 0.09 });
      }
    },
    owl(o, t) {
      for (const [dt, f] of [[0, 392], [0.42, 370], [0.62, 370]]) {
        mTone(o, t + dt, { wave: 'sine', freq: f, to: f * 0.94, slide: 0.25, gain: 0.043, attack: 0.06, decay: 0.28, cut: 900 });
      }
    },
    frog(o, t) {
      for (let k = 0; k < 3; k++) {
        mTone(o, t + k * 0.13, { wave: 'sawtooth', freq: 140 + WS.random() * 30, gain: 0.026, attack: 0.01, decay: 0.07, cut: 700 });
      }
    },
    crow(o, t) {
      for (let k = 0; k < 2 + WS.floor(WS.random() * 2); k++) {
        mTone(o, t + k * 0.32, { wave: 'sawtooth', freq: 640, to: 500, slide: 0.18, gain: 0.020, attack: 0.02, decay: 0.18, cut: 1500 });
      }
    },
    // A hawk high over dry ground: one long, keening fall.
    hawk(o, t) {
      mTone(o, t, { wave: 'sawtooth', freq: 2900, to: 1900, slide: 0.55, gain: 0.045, attack: 0.04, decay: 0.6, cut: 4200 });
      mTone(o, t + 0.75, { wave: 'sawtooth', freq: 2700, to: 1800, slide: 0.4, gain: 0.03, attack: 0.03, decay: 0.45, cut: 4000 });
    },
    wail(o, t) {
      mTone(o, t, { wave: 'sine', freq: 330, to: 520, slide: 1.2, gain: 0.017, attack: 0.8, decay: 1.8 });
      mTone(o, t + 1.2, { wave: 'sine', freq: 520, to: 290, slide: 1.4, gain: 0.014, attack: 0.1, decay: 1.6 });
    },
    cicada(o, t) {
      /* A pulsing buzz: the carrier through a tremolo stage (which swings
         between silence and full), then an envelope that swells and fades.
         The tremolo has to MULTIPLY the envelope, not add to it - summed into
         the same gain it swung the level by half of full scale around a
         target of a hundredth, and the cicadas came out as the loudest thing
         on the plain. */
      const ctx = Audio.ctx;
      const carrier = ctx.createOscillator(), trem = ctx.createOscillator();
      const depth = ctx.createGain(), am = ctx.createGain(), env = ctx.createGain();
      carrier.frequency.value = 4800 + WS.random() * 900;
      trem.type = 'square'; trem.frequency.value = 55 + WS.random() * 25;
      am.gain.value = 0.5; depth.gain.value = 0.5;
      trem.connect(depth); depth.connect(am.gain);
      carrier.connect(am); am.connect(env); env.connect(o);
      const len = 1.6 + WS.random() * 1.4;
      env.gain.setValueAtTime(0.0001, t);
      env.gain.linearRampToValueAtTime(0.03, t + len * 0.4);
      env.gain.linearRampToValueAtTime(0.0001, t + len);
      carrier.start(t); trem.start(t); carrier.stop(t + len + 0.05); trem.stop(t + len + 0.05);
    },
    gust(o, t) {
      mNoise(o, t, { filter: 'lowpass', freq: 220, to: 520, q: 0.9, gain: 0.040, attack: 1.8, decay: 2.4 });
    },
    whistle(o, t) {
      const f = 620 + WS.random() * 260;
      mTone(o, t, { wave: 'sine', freq: f, to: f * 1.25, slide: 1.6, gain: 0.014, attack: 1.0, decay: 1.8 });
    },
    chime(o, t) {
      for (let k = 0; k < 3 + WS.floor(WS.random() * 3); k++) {
        const f = 1800 + WS.random() * 2200;
        mTone(o, t + k * (0.15 + WS.random() * 0.2), { wave: 'sine', freq: f, gain: 0.012, attack: 0.003, decay: 1.4 });
      }
    },
    creak(o, t) {
      mTone(o, t, { wave: 'sawtooth', freq: 90, to: 130, slide: 0.6, gain: 0.026, attack: 0.05, decay: 0.6, cut: 500 });
    },
    shimmer(o, t) {
      for (let k = 0; k < 4; k++) {
        const f = 3000 * Math.pow(1.38, k) * (0.95 + WS.random() * 0.1);
        mTone(o, t + k * 0.12, { wave: 'sine', freq: f, to: f * 1.12, slide: 1.4, gain: 0.009, attack: 0.3, decay: 1.6 });
      }
    },
    // Ice catching the light: a few very high, very short glints.
    glint(o, t) {
      for (let k = 0; k < 4 + WS.floor(WS.random() * 4); k++) {
        mTone(o, t + k * (0.06 + WS.random() * 0.12), { wave: 'sine', freq: 5600 + WS.random() * 2600,
          gain: 0.009, attack: 0.002, decay: 0.35 });
      }
    },
    // A great bell struck somewhere under the arena, out of tune with itself.
    toll(o, t) {
      for (const [m, g, d] of [[1, 0.05, 3.2], [2.02, 0.022, 2.2], [2.94, 0.012, 1.6]]) {
        mTone(o, t, { wave: 'sine', freq: 146.8 * m, gain: g, attack: 0.004, decay: d, detune: 12 });
      }
    },
    deep(o, t) {
  ...
  function life(state, t0) {
    ...
    const fn = LIFE[kind];
    if (!fn) return;
    const pan = (WS.random() * 2 - 1) * 0.8;
    fn(lifeOut(state.bed, pan, LIFE_ROOM[kind] || 0.3), t0 + WS.random() * 0.3);
  }
  ...
  function airBed(score, bed) {
    /* The weather bed is gone. It was two loops of filtered white noise under
       every battlefield, forever, and however it was filtered, drifted or
       made to gust, it was heard as one thing: a hiss. Places are made of
       what lives in them now - see LIFE below. */
    return null;
  ...
  Audio.startScore = function (key) {
    ...
    zone.gain.value = 1;
    zone.connect(bed);
    /* The melody and the chords play in a room. Dry, the score was a synth
       in a box; a modest send puts the notes somewhere and lets a phrase
       end on its own tail instead of stopping dead. */
    send(zone, 0.28, true);
    ...
    state.timer = setInterval(() => {
      ...
      while (state.nextTime < Audio.ctx.currentTime + 1.0) {
        ...
        if ((s % 2 === 0 || state.intensity > 0.5) && !rests) {
          ...
          mTone(lead, t0, { wave: 'sine', freq: f * 2, gain: 0.014, attack: 0.005, decay: beat * 0.5 });
        }
  ...
  function bossVoices(state, t0, s, beat) {
    ...
    /* A rattle on the off-beats: the top of the mix, where the zone has
       ...
       than fast. */
    if (s % 2 === 1 && !dread) {
      mNoise(g, t0, { freq: 3200, to: 2400, decay: 0.06, gain: 0.030, q: 1.5 });
    }
  ...
  /* -------------------------------------------------------------- voices --
   * THE BOSSES TALK. Not in words - nothing in this game is voiced and
   * nothing will be - but the way a character in a big fantasy game grunts
   * or harrumphs or cackles as their line comes up: a short, silly burst of
   * speech-shaped noise that says WHO is talking and HOW they feel about it,
   * so a line lands with a voice behind it instead of in silence.
   *
   * It is a small vocal tract. A buzz (the voice box) at the speaker's pitch
   * goes through three resonant filters set to a vowel's formants (the
   * mouth), and the formants glide from vowel to vowel as the syllables go
   * by, with a puff of consonant at the front of some of them. The vowels
   * are taken from the line itself, so the same line always sounds the same
   * and a longer line babbles a little longer. How it is punctuated sets the
   * mood: a shout climbs and hits harder, a question lifts at the end, a
   * trailing line sinks and fades.
   *
   * Every speaker is a recipe: how high, how big a mouth, how fast, and
   * what extra it carries - Grimtunnel's nasal squeak, Mordecai's hollow
   * echo, Marrowfrost's choir of cold under a voice deeper than anything
   * else in the game. */
  const VOWELS = {
    a: [730, 1090, 2440], e: [530, 1840, 2480], i: [300, 2200, 2950],
    o: [570, 840, 2410], u: [320, 870, 2240], y: [400, 1700, 2500],
  };
  const VOICES = {
    // A goblin foreman in a hurry: high, quick, nasal, all over the place.
    grimtunnel: { pitch: 300, mouth: 1.35, syl: 0.072, count: [5, 9], wobble: 0.24, gain: 0.45, nasal: 1100, room: 0.12 },
    // A pirate captain: brassy, confident, falling at the end of a line.
    admiral: { pitch: 210, mouth: 1.12, syl: 0.1, count: [3, 5], wobble: 0.12, gain: 0.45, room: 0.16 },
    // A necromancer who died: low, slow, hollow, with an echo in him.
    ...
    gurgle: { pitch: 240, mouth: 1.25, syl: 0.085, count: [4, 7], wobble: 0.3, gain: 0.42, bubble: true, room: 0.15 },
    machine: { pitch: 110, mouth: 0.95, syl: 0.13, count: [3, 4], wobble: 0, gain: 0.39, square: true, room: 0.2 },
  ...
  function syllablesOf(text, count) {
    const out = [];
    const clean = text.toLowerCase().replace(/[^a-z]/g, ' ');
    const groups = clean.match(/[aeiouy]+/g) || ['a'];
    const want = WS.clamp(groups.length, count[0], count[1]);
    for (let i = 0; i < want; i++) {
      const g = groups[WS.floor(i * groups.length / want)];
      const v = g[0];
      // A consonant ahead of it if the letter before this group in the line
      // was one: that is what makes it babble rather than moan.
      out.push({ v: VOWELS[v] ? v : 'a', stop: i > 0 && (i * 7 + g.length) % 3 !== 0 });
    }
    return out;
  }
  ...
  Audio.babble = function (who, text, opts) {
    ...
    send(out, v.room);
    const mouth = [0, 1, 2].map((k) => {
      const f = ctx.createBiquadFilter();
      f.type = 'bandpass';
      f.Q.value = [5, 8, 11][k];
      const g = ctx.createGain(); g.gain.value = [1, 0.55, 0.28][k];
    ...
    let into = amp;
    if (v.hollow) {
      // A short comb: the sound of a voice inside something it should not be.
      const d = ctx.createDelay(0.05), fb = ctx.createGain(), mix = ctx.createGain();
      d.delayTime.value = v.hollow; fb.gain.value = 0.55;
      amp.connect(mix); amp.connect(d); d.connect(fb); fb.connect(d); d.connect(mix);
      into = mix;
    }
    for (const f of mouth) into.connect(f);
    if (v.nasal) {
      const n = ctx.createBiquadFilter();
      n.type = 'peaking'; n.frequency.value = v.nasal; n.Q.value = 3; n.gain.value = 9;
      into.disconnect(); into.connect(n); for (const f of mouth) n.connect(f);
    }

    const srcs = [];
    const voice = (mult, detune, level) => {
      const o = ctx.createOscillator();
      o.type = v.square ? 'square' : 'sawtooth';
    ...
    voice(1, 0, 1);
    if (v.choir) { voice(2, 7, 0.35); voice(0.5, -5, 0.5); voice(1.5, 3, 0.18); }
    if (v.growl) {
      // A rasp riding the voice: noise through the same mouth.
      ...
      n.start(t0, Math.random() * 5); n.stop(end + 0.3);
    }
    ...
    syl.forEach((sy, i) => {
      ...
      let contour = shout ? 1.12 + 0.28 * Math.sin(k * Math.PI) : ask ? 1 + 0.35 * k * k
        : trail ? 1.05 - 0.3 * k : 1.06 - 0.16 * k;
      contour *= 1 + (Math.random() - 0.5) * 2 * v.wobble;
      if (v.bubble) contour *= i % 2 ? 1.3 : 0.85;
      for (const s2 of srcs) s2.o.frequency.setTargetAtTime(base * contour * s2.mult, ts, 0.02);
      const fm = VOWELS[sy.v];
      mouth.forEach((f, j) => f.frequency.setTargetAtTime(fm[j] * v.mouth, ts, 0.018));
      // Each syllable swells and nearly closes, which is what makes it speech.
      const peak = i === 0 ? 1 : 0.85;
      amp.gain.setTargetAtTime(peak, ts, 0.012);
      amp.gain.setTargetAtTime(0.12, ts + len * 0.72, 0.02);
      if (sy.stop) {
        noise({ bus: out, filter: 'bandpass', freq: 2800 * v.mouth, q: 1.4, attack: 0.001, decay: 0.018,
          gain: 0.25, delay: ts - ctx.currentTime, verb: 0 });
      }
    });
    amp.gain.setTargetAtTime(0.0001, end, 0.04);
    for (const s2 of srcs) {
      s2.o.frequency.setValueAtTime(base * s2.mult, t0);
      s2.o.start(t0); s2.o.stop(end + 0.4);
    }
    setTimeout(() => { try { out.disconnect(); } catch (e) { /* gone */ } }, (end - ctx.currentTime + 3) * 1000);

    // The score and the chatter step back while someone is talking.
    if (this.duck) {
      const t = ctx.currentTime;
      try {
        this.duck.gain.cancelScheduledValues(t);
        this.duck.gain.setValueAtTime(this.duck.gain.value, t);
        this.duck.gain.linearRampToValueAtTime(0.6, t + 0.08);
        this.duck.gain.setValueAtTime(0.6, end);
        this.duck.gain.linearRampToValueAtTime(1, end + 0.5);
      } catch (e) { /* raced a context change */ }
    }
  };

  /** Which voice a boss of the night speaks in: by name for the ones with
   *  a character, by what it is for the rest. */
  const BOSS_VOICE = {
    grimtunnel: 'grimtunnel', candlecrawler: 'grimtunnel', stormbreaker: 'grimtunnel', heart_drill: 'grimtunnel',
